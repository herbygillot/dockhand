...
Bump verifies and publishes by default, staying attached until the PR is confirmed. Separate verification is useful after a failure or corrective edit; it is not a mandatory extra build. `--to verified`, `--unverified`, and `--detach` stop earlier, skip the build, or let go. An optional explicit version remains supported. No public `--subport` is needed, and branch names and job IDs are optional selectors for exceptional cases, not required handoffs.
