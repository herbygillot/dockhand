...
Inference compares the whole selected source tree with the recorded contribution base. Every changed path must stay within that port directory or under `_resources`, since a port's update may change the PortGroup it loads; another port or an unrelated repository edit requires an explicit port. Rename detection is disabled so both sides of a move count. An unavailable base or changed evaluated target identity also requires explicit selection. This conservative first rule can require an explicit port after a rebase introduces upstream changes elsewhere. Untracked branches, detached checkouts, and zero/multiple recorded targets never guess from HEAD or the latest job.
