...
The device flow requires a registered GitHub OAuth application with device flow enabled. Development builds accept `--client-id` or `DOCKHAND_GITHUB_CLIENT_ID`; `make GITHUB_OAUTH_CLIENT_ID=<id>` embeds the public client ID in a release binary through `app.DefaultGitHubOAuthClientID`. No client secret is used or distributed. Login replaces the existing Dockhand credential for `github.com`.
...
The editor supports direct version declarations and evaluator-guided probes of literal inputs feeding calculated versions, including supported forge setup calls. It re-evaluates candidate inputs through MacPorts, requires an unambiguous upstream-ref mapping, and checks the resulting port version and edit fidelity; it does not require the evaluated port version to equal the tag text. See the [version-probing report](activity/2026-09-15-portedit-version-probing.md). Preparation resets a matching literal revision to zero, downloads each source archive from its unambiguous direct HTTP(S) master site, and rewrites checksum values with required sha256 and optional rmd160/size. A single checksums command may be unnamed for one archive or explicitly name each archive, including Tcl filename expressions. Tagged master sites can select different locations for different files; ambiguous mirror/site associations are refused. The primary port is selected; sibling metadata and dependencies must remain unchanged. The Go PortGroup’s recognized toolchain compatibility pre-check is allowed during archive preparation; MacPorts executes it during verification. Unknown pre-fetch hooks, any post-fetch hooks, and replaced fetch implementations require additional preparers. Patches are accepted only when proven to be regular local files within the frozen port directory. Calculated versions without a provable editable input, remote patches, ambiguous checksum groups, and unsupported vendored-source forms remain refused. Evaluation and fidelity apply to the selected platform and variants.
