...
| `adopt <branch> [port]` | Track a branch dockhand did not make as a contribution: one commit above master, one port directory, the port inferred from it unless named; `--squash` folds several commits into one, keeping the originals under a backup ref; `--dry-run` records nothing. |
