...
- **Coordinated through the database, blind across databases:** which contribution owns a port, the Tart capacity count, and the publication locks beside each database. Two databases on one checkout could both bump the same port, both believe one VM slot is free, and both push the same fork branch.

Two cheap guards: write the database path into the checkout under `.git` at registration so a command can notice a checkout another database owns, and move Tart slot accounting to a lease file under Tart's home so capacity is true for the machine.

On disk, nearly everything is either beside its database (the database and journal, `artifacts/tart`, the GitHub log cache, publication locks, backup staging) or named so two groups cannot collide (VM clones are `dockhand2-` plus a digest of pool and random request ID; branches carry a random job suffix; every capture, rebase, correction, and helper run uses a fresh temporary directory). The one gap is the Tart base images: `setup` takes no lock while provisioning or rebuilding, so a rebuild racing a clone is possible for any two processes, same database or not. A per-image file lock under Tart's home, exclusive for `setup` and shared for clones, closes it.
