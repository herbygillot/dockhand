...
Schema 5 adds `jobs.reused_attempt` and `jobs.reuse_detail`, plus indexes for repository/tree/target lookup. This slice initially made the store check the reference's repository, passing outcome, source tree, completed job state, and absence of the referencing job's own attempts. The later [state-policy boundary pass](2026-09-14-state-policy-boundary.md) moved eligibility checks into workflow; the store now preserves repository scope and reference immutability. Full applicability policy remains in `verify`. No evidence cache table, provider operation, extra package, dependency, or global lock is introduced. Writable opening migrates prior schemas transactionally; read-only opening still requires the current schema.
