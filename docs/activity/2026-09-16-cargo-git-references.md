...
- Live: master already carried Codex 0.154.0, so the replay bumped Codex to 0.155.0-alpha.15 on master commit `bffe10341a5`. Preparation completed: version and revision edited, main checksums refreshed, `cargo.crates` regenerated, sixteen `rev=`-pinned crates reported as left to online resolution, and the pinned V8 auxiliary archives untouched. The ordinary workflow then verified the change in the Tahoe Tart guest as job `job_OSMQSWZYK5L7JG2Q2BRIPVG2J7`: the prepared branch was transferred, every registry crate fetched, and the build failed in the `patch` phase because the port's `patch-package-managed-daemon.diff` no longer applies to the alpha source (four of five hunks rejected). That is prerelease drift in the maintained port, not a preparation defect, and it means Cargo's online resolution of the sixteen `rev=` crates was never reached in this run. The guest was released and the contribution abandoned. A build-phase confirmation of online resolution still needs a stable Codex release whose patches apply.
