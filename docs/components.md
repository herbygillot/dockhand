...
`ledger` owns the authoritative state ref, short transactions, common-directory locking, expected-ref checks, source pins, and derived Git notes. Notes export is part of this component; do not expose another authoritative note writer. Business decisions stay out of storage. `workflow` owns request intake and progression writes, including cross-record updates. Intake can run in the CLI process; after submission, drivers own progression and bookkeeping. Capability packages return results for the workflow engine to record.
...
Distinguish admitted, temporarily at capacity, unsupported, and submission-uncertain outcomes. A preliminary capacity check is advisory: actual admission must coordinate at the provider's resource scope, including other repositories sharing the same host. A repository ledger lock cannot enforce a machine-wide capacity limit by itself.
...
The initial package skeleton and selected Tcl/source-editing code are now present. The ledger now implements snapshot reads, bounded writer locking, guarded transactions, structural document validation, and source pins. `git` supplies the underlying object and ref operations. Driver execution, provider integrations, derived notes, and application wiring remain unfinished; their operational stubs still return `ErrNotImplemented`. The [groundwork activity report](activity/2026-09-10-groundwork.md) identifies imported and newly authored code and the checks performed. The initial [ledger implementation report](activity/2026-09-10-ledger.md) records subsequent implementation and runtime checks. The broader workflow validation scenarios above remain future work; no test suite has been imported or added.
