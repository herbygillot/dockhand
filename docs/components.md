...
`macports/dependency` owns Go/Cargo declaration parsing, source-manifest reading, optional helper discovery/execution, and validation of generated blocks. `prepare` composes these mechanisms with frozen-tree editing, archive hashing, and MacPorts reevaluation. Helper output is data: only recognized literal dependency declarations are incorporated, never a generated Portfile as executable Tcl. This introduces no workflow or state layer.
