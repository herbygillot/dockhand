...
`workflow.Resolution` is what one selection means for one action, fresh from master, continued from a prior job's recorded source, onto a contribution's current revision, or adopted from a hand-made branch, computed once by `Engine.Resolve` from the records, the repository, and master, and read by the bump, the preview, the verification, adoption, and the corrections. It is the one engine method that answers without a store, and it degrades explicitly, in its detail, when master is unreachable; see the [design](resolution-design.md).
