...
Tart removes its host `input.tar` after guest staging and durable admission; retries launch the same staged guest without retransferring the archive. Interrupted staging keeps its input until the reserved execution is closed and released. Normal preparation/staging scratch remains scoped to the operation and is removed on return, including errors, and what a process that died left behind is swept by the next command and by `gc`, as the next section says. Unknown leftovers from process death are not otherwise swept blindly.
...
## Transient and persistent directories

Dockhand keeps two kinds of directory apart. What a person may come back to lives under configured homes: the state database and its backups at `$HOME/.dockhand` by default, verification artifacts, host logs, and result files under the artifact directory beside the database, the Tart pool with its images and staged inputs, the GitHub verification log cache, and the PortIndex cache in the user cache directory. Nothing under those is removed except by the retention rules above.

Everything a command needs only while it runs lives under one run root in the system temporary directory, named `dockhand-run-` and a random suffix: materialized sources and workspaces, the overlays that candidate evaluations are made in, the git index and rebase directories of corrections, the sandboxes the dependency helpers and patch checks run in, and the buffer an editor opens for a message. A run root is created on first use, held under an advisory lock for the life of the process, and removed when the command ends, including on an interrupt. A process that dies without cleaning up, killed outright or lost with the machine, leaves its root behind with the lock released. The next command that opens a run root sweeps such roots first, and `gc` lists them, `--dry-run` as a preview, and removes them; a root whose lock is held belongs to a live process and is never touched. Directories that builds before run roots made directly in the temporary directory carry no lock, so `gc` removes those on age alone, once they are an hour old, which outlives any one operation of such a build; `--older-than` governs retained evidence and does not apply to them.
