...
### 1. Recognize versions carried as `*.setup` arguments

17,040 survey entries, 41% of the index, have no editable version input because the perl5, R, and ruby PortGroups carry the version inside `perl5.setup`, `R.setup`, or `ruby.setup`; recognizing that argument would move about 16,500 of them. Those families move less than python, but the share is too large to leave: a tool that cannot touch two fifths of the tree is not a tree tool. The version-input probe already recognizes `github.setup` and `go.setup` arguments, so this is the same recognition for three more commands with their argument positions, plus what follows from each PortGroup: their livechecks (CPAN, CRAN, RubyGems) already resolve through MacPorts' own checkers; the perl `p5-` stubs and their `p5.34-`/`p5.36-` subports are the python stub shape again and should reuse it; checksum blocks in those families are mostly legacy and already modernize. Validate with the corpus replay, now a half-minute run, and a real bump in each family.

### 2. Make branch cleanup durable and its status truthful
...
### 3. The four structural items from the architecture review
...
### 4. Finish the survey's tail

Three bounded pieces, each small: the qt family's revision is read from the same table as its checksums and the revision reset does not trace through tables yet, so a qt bump stops one step after the checksums; the qt4 PortGroup reads the installed Qt to pick a build layout and the five corpus ports that load it are unknown for it, which needs a rule for host reads that originate inside a PortGroup and land in build-only options; and about 110 ports whose only master sites are ftp, which the direct downloader does not speak.

### 5. State contracts: PR observation and queries
