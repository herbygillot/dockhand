...
### 2. State contracts: PR observation and queries
...
Two reviews on 2026-09-17 were triaged into the queue above on the same day: the [architecture review](reviews/2026-09-17-bump-workflow-architecture.md) contributed the four structural items, done that evening, and the session reuse in the throughput item, with its context-analyzer package and intake/driver split deferred until the rules and the engine's dependencies stop moving; the [state-boundaries review](reviews/2026-09-17-workflow-state-boundaries.md) contributed durable branch cleanup, done the same evening, and the state-contracts item, with its narrower reads deferred behind measurement. Its claims were checked against the tree before being taken: the disposition is committed before branch retirement, the view reports cleanup unconditionally, the observation throttle is a process-local map, the due-ordered query pages by ID, and the working set compares by deep equality.
