...
Reconciled on 2026-09-17 (evening) from the remaining survey item, the two reviews of that day ([architecture](reviews/2026-09-17-bump-workflow-architecture.md), [state boundaries](reviews/2026-09-17-workflow-state-boundaries.md)), and the follow-ons the day's work surfaced. The order weighs, in turn: correctness gaps a person has already hit, share of the tree unblocked per unit of work, and structure that makes the next coverage work cheaper rather than more special-cased.

### 1. Make branch cleanup durable and its status truthful

From the state-boundaries review, and seen in practice earlier the same day when merged branches hung around. `refreshChange` commits the merged disposition and only then retires branches; the notes cleanup returns are appended to the command result, not stored, and the view says "merged; branches cleaned" whether or not deletion happened. A process exit after retirement, or a failed remote deletion, leaves a merged contribution that nothing revisits, since observation selects open contributions and refresh of a retired one returns early. Record a small branch-cleanup obligation atomically with retirement, local and remote outcomes tracked separately as pending, complete, or kept for a reason, with the expected published commit rechecked before deletion so a moved branch stays protected. Reuse the claim and retry vocabulary; keep contribution branches distinct from provider VM resources. Status then distinguishes merged from cleaned.

### 2. The four structural items from the architecture review

Taken together as one item, in this order: an explicit final prepared result in place of reading the last fidelity report's snapshot (four sites do this today); a resolved bump selection that records the named port, the owning Portfile, the subport carrying the release, and the targets, so the stub redirect happens once instead of in binding, in the editor, and again on reselection; the preparation adapter's own request type with a single commit intent, so an option such as `--keep-old-checksums` is not threaded through five types; and the contribution view extracted from `workflow`, which has no dependency on the engine or the store. Fold the coverage-intent change in here: `RequiredTargets` takes a whole job spec and falls through to every buildable member when the initiating target matches none, the only path that yields the still-unexplained `py-idna` refusal; give it an explicit intent and make the fall-through an error. Each removes a hazard hit that day; none is large.

### 3. Finish the survey's tail

Bounded pieces, each small: the qt family's revision is read from the same table as its checksums and the revision reset does not trace through tables yet, so a qt bump stops one step after the checksums; the qt4 PortGroup reads the installed Qt to pick a build layout and the five corpus ports that load it are unknown for it, which needs a rule for host reads that originate inside a PortGroup and land in build-only options; about 110 ports whose only master sites are ftp, which the direct downloader does not speak; and the perl tail left by the [setup-argument work](activity/2026-09-17-setup-version-inputs.md): 32 modules spelled `v1.2.3`, which the stability classifier calls unknown and so need an explicit version, and the handful of livechecks whose CPAN author directory redirects from https to http, which the fetch guard refuses.

### 4. State contracts: PR observation and queries

From the state-boundaries review. Give PR observation a persisted next-attempt time and a bounded query for due open contributions, so separate drivers do not poll the same PR and a restart does not lose the throttle; let explicit refresh and periodic observation share one recording path. Replace the catch-all `state.Query` with operation-shaped queries (`DueJobs`, `JobHistory`, `OpenContributions`, `CleanupCandidates`): a due-ordered result paged by ID can skip records, a contract hazard the current cycle avoids only by not paging. The explicit working-set change set from the same review is a maintenance hazard rather than an incident and can follow.

### 5. Throughput's remaining levers

After item 2, which gives it a home: reuse one native interpreter session across the resolve, baseline, candidate, and final evaluations of a preparation, never across a modeled observation, with a differential guard test that evaluates the corpus through fresh and shared sessions and compares snapshots; then test operand and regeneration behavior at their unit seams with one or two end-to-end cases each.
...
### go.toolchain_min for git-fetched module-mode ports

The toolchain rule reads `go.mod` from the kept archive; a git-fetched port has none on the host, so vault-shaped ports get no comparison. Reading the manifest from the forge at the resolved commit would close it. Small, and only worth it once such ports are bumped regularly.
...
- **Small fixes on hand:** `version.Info.String()` repeats the revision when Go stamped a pseudo-version; the GitHub log-retention test's one-time load flake now reports what prune returned and should be closed or fixed when it recurs; the corpus controls are `terraform-1.16 1.16.2`, `deno` at any version but the current, `codex 0.155.0-alpha.15`, `wasmer 7.4.2`, `gh 2.83.0`, and `helm-4.2` rather than `helm`, which is the series metaport. Do these inside whichever item touches the area.
...
Two reviews on 2026-09-17 were triaged into the queue above on the same day: the [architecture review](reviews/2026-09-17-bump-workflow-architecture.md) contributes Next item 3 and item 6's session reuse, with its context-analyzer package and intake/driver split deferred until the rules and the engine's dependencies stop moving; the [state-boundaries review](reviews/2026-09-17-workflow-state-boundaries.md) contributes Next items 2 and 5, with its narrower reads deferred behind measurement. Its claims were checked against the tree before being taken: the disposition is committed before branch retirement, the view reports cleanup unconditionally, the observation throttle is a process-local map, the due-ordered query pages by ID, and the working set compares by deep equality.
