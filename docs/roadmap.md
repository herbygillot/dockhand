...
### 1. Replay the coverage corpus and settle the shared-release publication refusal

Three coverage changes landed on 2026-09-17 without the corpus replay this document requires for them: legacy checksum blocks are [modernized when touched](activity/2026-09-17-checksum-modernization.md), an already-current archive release [reports no update](activity/2026-09-17-checksum-modernization.md), and a deployment-target reassignment is a [benign sink](activity/2026-09-17-deployment-target-sink.md). Each was checked against its own refused population and against the ports it was built from. Run the 147-Portfile comparison and the Terraform/Helm/gh/Deno/Wasmer controls, and report input discovery, candidate checks, archive preparation, and builds separately. In the same pass, make the second `py-idna` run's publication refusal ("missing shared-release target py310-idna") reproducible or close it: the recorded job and attempt pass every policy today and the path is pinned by a test, so the cause is still unknown; see the [stub bump report](activity/2026-09-17-stub-bumps.md).

### 2. Trace checksum values held in data tables
...
### 3. Recognize versions carried as `*.setup` arguments

17,040 survey entries, 41% of the index, have no editable version input because the perl5, R, and ruby PortGroups carry the version inside `perl5.setup`, `R.setup`, or `ruby.setup`; recognizing that argument would move about 16,500 of them. It ranks after the two items above because those families move less than python: 780 perl bumps a year against 2,440 python ones, and ruby and R far fewer. The version-input probe already handles command-substitution arguments, so this is an input-recognition extension with the same fidelity checks, followed by the corpus replay.
