...
The initial implementation supplies each of the six stages above within the supported cases below. `assess` and `bump` share scoped planning. `assess terraform-1.16 --version 1.16.2` checks an explicit archive version; `bump terraform-1.16 1.16.2 --dry-run` also downloads the affected archives and previews the completed edit. No forge identity or Git commit is manufactured for HashiCorp's archive source.
