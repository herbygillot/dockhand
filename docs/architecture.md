...
The decision, plan, completed job, explanation, and original-attempt reference commit together. Reuse creates no attempt, submission, or VM and preserves original evidence and independent cleanup. Cancellation already applied to the job takes precedence. Status reads the original execution in its consistent snapshot. Reattachment does not reconsider a completed job's decision.
...
Acceptance records the job and immutable publication intent together. The action binds the source commit, target/base/head repositories and branches, URLs, exact expected remote head, observed PR metadata, desired content, and evidence reference. Execution uses the job's existing lease/generation, rather than creating another claim vocabulary. An active-action uniqueness constraint reserves each forge/head-repository/head-branch across all repository entries in the DB.
