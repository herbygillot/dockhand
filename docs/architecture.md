...
`app.PreviewPreparation` constructs Git and MacPorts capabilities without SQLite or a verification provider, and the CLI renders its result. `bump-revision --diff` exercises the real capability. Driver preparation claims, branch integration, and verification continuation remain unimplemented. This path does not use the existing exclusive branch/target association, leaving that limitation to be corrected before durable preparation jobs are connected.
