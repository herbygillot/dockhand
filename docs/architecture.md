...
Automatic selection reads every catalog page within a fixed bound, filters the supported source convention, applies the evaluated Tcl livecheck capture to candidate archive URLs, and compares versions with MacPorts `vercmp`. It is a constrained discovery capability, not a replacement for all MacPorts livecheck types. Failed, incomplete, empty, or ambiguous evidence remains unknown. Catalog pagination is an observation over multiple HTTP requests, not an upstream snapshot; detected duplicate tags are rejected, but an upstream change during pagination cannot always be detected.
