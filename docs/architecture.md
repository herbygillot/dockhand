...
Current-checkout verification freezes tracked working-tree contents, including staged additions, without modifying the user's index or branch. Explicit branch selection freezes committed contents. Git supplies snapshot mechanics, MacPorts evaluates that complete snapshot, and workflow accepts the immutable input with its selection provenance. No provider reads a mutable checkout during a queued or running build. Detect conflicting source changes during capture rather than knowingly accepting an inconsistent snapshot; source availability and recovery follow the existing rules above.
...
The verification path handles one resolved target from standalone committed source, an existing tracked revision, or the result of a version- or revision-bump job. The accepted `JobSpec.Build` records a `BuildConfig`: provider identity, complete platform, immutable environment digest, source-build choice, and explicit test policy. `BuildSpec` combines that configuration with the selected revision, source, target, and concrete artifact inputs. `verify.PlanSingle` constructs this first coverage plan without calling MacPorts or the provider. A missing configuration, missing committed source, or multiple targets produces a needs-attention outcome. Unsupported preparation forms and publication still produce recorded needs-attention outcomes when a cycle encounters them. The broader planner and dependent coverage remain separate future work.
...
The provider hashes base-image contents and verifies the accepted platform in the guest. It transfers the exact committed Git tree, builds an index from that tree, and sets it as the sole MacPorts source and default resource tree. The guest executes lint, an explicit `port -d build`, declared tests when requested, and install, recording structured phase evidence and dependency-failure context. No unrelated-dependency failure is automatically attributed to the candidate change.
...
The upstream groundwork provides deterministic explicit-release matching separately from evidence collection. A known tag pattern or an unambiguous current-version/current-tag pair supplies prefix/suffix inference. Candidate selection preserves explicit prefixes and checks supplied evidence for missing, ambiguous, or inconsistent matches. The evaluator exposes computed Git/GitHub source options for this integration. Exact GitHub tag lookup, bounded version editing, downloads/checksums, and a durable resolved-release checkpoint are now implemented. Automatic selection now uses GitHub release/tag catalogs and native MacPorts filtering and comparison.

## Durable revision-bump execution

`workflow.BindPreparation` selects one committed source and records its branch, exact source IDs, target, platform, and Git author. `app` captures verification settings or a setup problem without making provider availability a prerequisite for branch creation. `Submit` accepts immutable intent; no contribution branch exists yet. The CLI then drives the same engine used by `wait` and `start`.
...
`forge` holds shared remote facts and access contracts; `forge/github` translates GitHub names, URLs, HTTP responses, and errors into those facts. Its repository object binds tag lookup, release/tag catalogs, and public URLs to one validated name. A client may bind multiple independent repositories. Binding performs no HTTP, and `upstream.RepositoryReader` makes this boundary explicit without importing the GitHub adapter. `go-github` supplies the API implementation inside the adapter; its types and protocol choices do not enter workflow or upstream policy. API errors retain the SDK error chain, including rate-limit details. PR writes are not automatically retried, and uncertain outcomes remain subject to workflow reconciliation.
