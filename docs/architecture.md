...
Tracked verification can add a revision containing a tree-only source without changing the contribution's edited targets. A detached checkout remains standalone. Identical trees in later committed revisions can eventually support evidence reuse, but this slice does not implement that judgment or publication. Tart verifies optional commit/tree consistency, then creates the guest archive from the accepted tree for either input kind.
