...
## Explicit version preparation

Version and revision bumps share binding, claimed preparation, candidate commit creation, guarded integration, and verification continuation. Version bumps add one earlier claimed pass: `prepare.ResolveRelease` evaluates the frozen source and asks `upstream.Resolve` to resolve the explicit request. GitHub's adapter reads the exact reference and peels annotated tags to commits. The workflow checkpoints `record.Release` before it can invoke preparation or download an archive. The checkpoint keeps requested spelling, Portfile version, repository, tag, commit, and observation time distinct. Accepted intent stays immutable.
