...
Standalone publication binds one tracked branch, adopts committed edits as a new immutable revision when needed, and requires passing verification for the complete tree and edited target. The most recent terminal attempt for that tree/target supplies the configuration to publish; it is not silently replaced with an older passing platform/configuration. The shared applicability policy checks its complete inputs, and the driver rechecks for newer matching negative evidence before fresh external effects. This first path does not schedule missing verification.
