...
Under one ledger transaction, intake checks request identity and referenced records, then writes a queued job and its request-to-job index. The receipt is returned only after the transaction succeeds; any error, including an uncertain commit outcome, returns no successful receipt. An identical retry returns the original job ID and acceptance time. Reusing the ID with different intent is an error. Retries compare against the immutable accepted spec before checking current change disposition, so later job progress, revision advancement, or change closure does not break a prior receipt. Ledger encoding and decoding enforce the bidirectional request/job index, and writes retain source pins while checking commit/tree agreement.
