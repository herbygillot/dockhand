...
Revisions have an optional `release_scope` JSON value. It records the edited literal's source location, affected and protected targets, before/after version and archive identities, and which parents are metadata only. Candidate checkpoints carry the same scope until branch integration. Correction, adoption, and an update prepared onto a contribution preserve membership, settled before the branch moves; a revision recorded without a scope, an adopted branch's, may be followed by a scoped one, which fixes the membership from then on; immutable prior revisions retain their evidence. `Change.Targets` and accepted job targets retain the initiating command target, while the revision scope supplies the complete verification cohort. Scope is independent of reverse-dependent discovery.
