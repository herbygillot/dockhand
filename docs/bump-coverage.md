...
The implemented recognition covers the general shape of Wasmer's already-registered unconditional diagnostic/error hook, independent of port name. Accept only the known MacPorts scope wrapper, permitted diagnostic statements, safe message substitutions, and an unconditional error return. Do not execute fetch hooks to discover whether they mutate something. Unknown commands, command substitutions in messages, array-index substitutions, writes, URL/distfile mutations, replaced fetch procedures, and post-fetch work remain unsupported. Preserve the existing narrowly checked Go toolchain guard. Since 2026-09-22 a command those rules refuse is judged by its effect on the fetch: the worker ships the definitions of the commands a hook calls, and a command that demonstrably writes none of the fetch's inputs, `macports.AffectsFetch`, is admitted, a procedure by its body; `exec` and its kind, computed names, and undefined commands stay refused, and the refusal names what the command does and where ([note](activity/2026-09-22-guard-effect-rule.md)).
...
Implemented through source-bound scalar writes and option results in `eval`, with profile selection in `portedit`. Native operand observations retain source frames without repeating getters. The source scanner remains a backstop for unvisited branches. Supported comparisons include inverted forms; aliases and formatting use bounded Darwin-major metadata enumeration. Mutable, external, or unresolved thresholds remain explicit limitations. Reads of the minor version, macOS version, or deployment target are classified by position: text in build-phase commands and conditions guarding configure-only branches are harmless, while reads that can reach a version, distfile, or checksum declaration are refused with the command named. See the [implementation report](activity/2026-09-16-platform-observations.md) and the [classification report](activity/2026-09-16-unmodeled-read-classification.md).

py-openssl's named subport, libfec, mpir, abendrot, bun, warzone2100, fldigi, mrustc, and git-devel reach local input assessment. Reads of the selected compiler or its flags are classified by position like the minor-version reads, with a variable set from such a read carrying it to its uses; a host probe raised by MacPorts compiler selection during a modeled evaluation is tolerated only when every such read is in a build position. Pre-fetch hooks that only reject unsupported configurations, such as the perl5 PortGroup's required-variant check, are recognized guards.
