...
An obsolete main port that is `replaced_by` the subport being bumped and carries that subport's version as its own literal, the shape of the terraform Portfile, moves with the bump and appears as a metadata-only member of the release scope; nothing else in the Portfile is touched without `--shared-release`.
