...
A verification builds on this Mac's macOS release. `--os` names other releases to build on instead, as `setup --os` names them (`sonoma` or `14`); repeat it for several, and `--os available` names every release with a prepared `dockhand-base-<release>` or `dockhand-xcode-<release>` image. The host is built on only when it is named too. Each release builds in its own image, all of them share the Tart capacity, and the verification passes only when every release passes; each release's result is recorded as that release's evidence. The port is still evaluated on the host, so its targets, variants, and full-Xcode need are the host's. `--os` is Tart's: it is refused with `--provider github`, whose workflow builds on the fork's runner matrix, with `--image`, which names one release's image, and with `--dependents`, which is planned on one platform. It applies to that verification only; a later `verify` without it builds on the host again. See [build platforms](build-platforms.md).

```sh
dockhand verify jq --os sonoma --os sequoia --os tahoe
