...
Git dependencies require HTTPS GitHub repositories, an explicit branch, and a full commit hash. These map to MacPorts' `cargo.crates_github` source replacement format. Tags, `rev` selectors, default-branch references, non-GitHub repositories, and conflicting branches for one repository require manual preparation. Dockhand evaluates provisional declarations to obtain the PortGroup's actual archive locations, downloads those archives, and fills their SHA-256 checksums. It does not invent a second GitHub archive URL convention.
