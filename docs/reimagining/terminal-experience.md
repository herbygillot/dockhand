...
A **changeset** is a branch of the MacPorts ports tree: a base commit on `master`, plus an ordered list of commits above it that together touch one or more ports. It becomes one pull request. On screen, dockhand calls it what contributors already call it, a **branch**, and the rest of this page does too ([§11](#decided-2026-09-24)).

Inside a branch, **each commit is one logical change**, nearly always to one port: `jq: update to 1.8.1`, or `gdal: rebuild for poppler 25.09.0`. The MacPorts guide asks contributors to "minimize the number of commits … ideally with one commit per logical change". It also says that when a reviewer asks for changes, "please squash them to avoid unnecessary commits." Dockhand keeps that shape by construction. An edit to a port the branch already changes is folded into that port's commit, not stacked on top of it.
...
- **Submit** is the one verb that acts on GitHub for you: it pushes to your fork and opens or updates the pull request. It refuses a branch that is untested or misshapen unless you tell it otherwise.
...
1. **The branch is the unit of work.** Everything you author lands in one, and everything dockhand shows is arranged by branch. A port is what you edit, a commit is what a reviewer reads, and a branch is what you ship.
...
4. **Git stays the truth, and stays yours.** Each branch is a real Git branch with its own worktree, so `git log`, `git diff`, and your editor all work. Use Git however you like; dockhand reads the branch and reconciles. Your main checkout is never touched.
...
6. **Speak the contributor's language.** Output uses port names, versions, pull request numbers, macOS release names, and branch names. It never shows job or attempt IDs at the default level. Commit subjects are written the way maintainers write them.
...
| --- | --- | --- |
| **branch** | A branch of the ports tree that dockhand tracks, with its own worktree, commits, test results, and at most one PR. This is the design's changeset. | Its name (`jq-1.8.1`), a port it changes (`jq`), or its PR (`#34901`). |
| **commit** | One logical change: to one port, or the same rebuild across several. Dockhand knows what kind of change it is: update, rebuild, checksums, new port, or other. | Rarely named directly; `status <branch>` lists them. |
| **port** | A port or subport, as MacPorts names it. | `jq`, `py313-ipdb`, `terraform-1.16`. |
| **test** | Builds of a branch's ports by one or more providers, at one exact state of the branch. | By branch; `--on` picks the providers. |
| **provider** | A way of building a branch: a clean VM, a MacPorts prefix on this Mac, GitHub Actions in your fork, or a command of your own. Its verdict is what "passes" means. | `tart`, `prefix`, `github`, or a name you configure. |
| **queue** | Every requested build, in order, for all your branches. | `dockhand queue`. |
...
| **PR** | The pull request a branch became. | `#34901`, or its URL. |

Some words are gone from the user's view: job, attempt, contribution, revision, change ID, and evidence. They remain real in the engine, and `-v` and `--json` show them. At the default level, a person never needs one to type a command.

### Where branches live

Each branch that dockhand creates gets a **sparse worktree** of your ports clone. By default it lives in a visible directory beside the clone, `~/src/macports-branches/<name>`, which `init` offers and `branches` in the config moves. A worktree holds `_resources` and the directories of the ports the branch touches, and it grows when you edit another port in it. So ten open branches cost a few megabytes, not ten copies of a 40,000-port tree, and your main checkout stays on whatever you left it on.

The worktrees are ordinary directories and are meant to be used directly. Open `~/src/macports-branches` in an editor and every open branch is there side by side. Edit any file in one and run `dockhand save`, or run `git` in it as you would anywhere. Dockhand removes a branch's worktree only after its PR merges or you `drop` it, and never while it has unsaved edits.

A branch you made yourself stays where you made it. If you run a dockhand command on it, dockhand offers to track it, and its worktree is your checkout.

The **current branch** is the one whose worktree you are in, or the branch checked out in the directory you're in. Outside any branch, commands take a selector, and on a terminal they offer a picker when the choice is ambiguous. Dockhand never guesses from "the one you used last".
...
| **Test** | ★ `test [<port> [+variant…]]…` | Queue builds of the branch's ports by the providers in `--on`. Stays and shows progress; Ctrl-C detaches. |
...
| **Branches** | ★ `status [<branch>]` | Everything open and what it needs from you, or one branch in detail. Plain `dockhand` does the same. |
| | `watch [<branch>]` | The same view, live: the queue, builds as they run, and keys for the verbs. |
| | `new <name>` | Start an empty branch from fresh `master`, for work you'll author by hand or across several ports. |
| | `track <branch>` · `fetch <pr>` | Track a branch you made, or fetch someone's open PR as a tracked branch. |
| | `rebase` · `rename` · `drop` | Move onto current `master`, rename, or stop pursuing a branch (`drop` removes the worktree and keeps the Git branch and its history). |
| | `path` · `shell` · `open` | Get to a branch's worktree. |
...
| | `info <port>` | One port: versions, maintainers, open branches and PRs, and what dockhand can automate for it and why. |
| | `dependents <port>` | What depends on a port, and how (library, build, run). |
| **Queue and server** | `queue` | The queue across all branches. Subcommands cancel, retry, and move items. |
| | `server` | Run, start, stop, install at login, or inspect the local server. |
| **Setup** | `init` | First-run setup: MacPorts, GitHub login, fork, clone, providers, and config. Safe to rerun. |
| | `doctor` | Check all of that, and say what to fix. |
| | `provider` | List, add, set up, and check the ways builds run: Tart images, a MacPorts prefix, GitHub Actions in your fork, or your own command. |
...
Everyday
  ...
  save        fold your edits into the branch
  test        build the branch's ports
...
Test          logs · watch
Branches    new · track · fetch · rebase · rename · drop · sync · path · shell · open
Discover      outdated · info · dependents
Server        queue · server
Setup         init · doctor · provider · auth · config
...
Let's get you ready to contribute to MacPorts.
  ...
  Branches     ? keep branches beside it, in ~/src/macports-branches? [Y/n] y
  Your ports   ✓ 212 ports list @ada as a maintainer
  Providers    github  ✓ MacPorts' CI workflow is enabled in ada/macports-ports
               tart    ! no images yet: dockhand provider setup tart     (macOS 26 Tahoe, ≈ 30 min, 45 GB)
               prefix  · optional: dockhand provider setup prefix        (quick builds on this Mac)
               builds use github until tart is set up
...
jq 1.7.1 → 1.8.1   (GitHub tag jq-1.8.1)
  ✓ branch jq-1.8.1, from master 4c1e2d0
...
```console
$ dockhand edit openjdk21
  ✓ branch openjdk21, from master 4c1e2d0
  ...
  ✓ lint, evaluation
  ✓ commit  openjdk21: update to 21.0.9      (branch renamed openjdk21-21.0.9)
next: dockhand submit openjdk21
```

`save` is what runs when the editor closes. It works the same after edits in any editor or IDE: run `dockhand save` in the worktree. Filling in checksums after a hand edit is the job `port bump` has done since MacPorts 2.6. Dockhand does it in the branch, and follows it with the evaluation and lint checks.
...
croc 10.2.4 · the distfile changed upstream without a new name (stealth update)
  ...
  ✓ branch croc-10.2.4-checksums, from master 4c1e2d0
...
$ dockhand revbump gdal "rebuild for poppler 25.09.0"
  ✓ branch gdal-rebuild, from master 4c1e2d0
  ✓ revision 2 → 3
...
rift 0.4.2 · Rust (Cargo.toml) · MIT · "Fast structural diff for config files"
  ...
  ✓ branch rift-new, from master 4c1e2d0
...
### 5.7 A coordinated branch: a library and its dependents
...
poppler 25.08.0 → 25.09.0
  ✓ branch poppler-25.09.0, from master 4c1e2d0
...
next: dockhand revbump --dependents-of poppler     (from inside the branch)
```
...
On each platform, ports build in one VM, in dependency order, from the branch's own tree, so every dependent links against the new poppler and not a published binary. MacPorts CI does the same for a PR: one job per macOS release, building the changed ports from the PR's own tree.
...
```console
  ...
  ✗ caused by this branch: inkscape builds at master on sonoma
```

Dockhand builds the failing port at the branch's base to tell the two cases apart, because a dependent that was already broken is common. It reports `pre-existing: inkscape also fails at master` when that's the case. `--no-baseline` skips the extra build, and `test.baseline` in the config sets the default.
...
Only inkscape is rebuilt, and anything in the branch that depends on it. Every other port's result still applies, because nothing it builds from has changed ([§9](#9-what-this-asks-of-the-engine)).
...
$ dockhand tidy
update-jq isn't tracked yet: 5 commits above master 4c1e2d0, all changing jq, with PR #34905.
? track it? [Y/n] y
...
Prevention matters more than cure. On a tracked branch, `save` folds edits into the port's existing commit, and `submit` runs `check`, which refuses a misshapen branch with `run dockhand tidy`, or `--as-is` if you mean it. A contributor who only ever uses `edit`, `save`, and `submit` can't produce the stack at all.
...
pr-34905 · "Update jq" by @newcontrib · 5 commits · jq · maintainers can edit
  ✓ branch pr-34905 at ~/src/macports-branches/pr-34905
...
6 ports: 5 branches, 1 left for you (unrelated ports go in separate PRs)
...
4 branches are on their way to a PR; you can close this terminal.
next: dockhand watch
```
...
Every branch goes through the same gates on its own. It has to pass `check`, then build and install on the platforms in `test.on`, and only then is the PR opened. One failure stops only its own branch, never the batch. The server carries each branch through, so closing the terminal loses nothing, and the one PR per unrelated port that reviewers want comes out naturally.
...
With that, a plain `dockhand update --outdated --mine`, or `dockhand update jq`, carries every branch that passes through to a PR. `--stop` on any one run holds it at the local checks.

The server can run the same loop every morning by itself, and `server.updates` sets how far it goes. This design recommends stopping one step short of the PR:

```toml
[server]
updates = "test"     # list · draft · test · submit
```

Overnight, each new release becomes a branch, goes through `check`, and is built by your providers. In the morning, the ones that passed wait for one quick look each:

```console
$ dockhand submit --passing
3 branches passed overnight; 1 didn't (dockhand status)
  y submit · n not now · d diff · r release notes

bat-0.26.0       0.25.0 → 0.26.0 · tart:tahoe ✓ · Portfile +3 −3, cargo.crates +12 −9
? submit? y      ✓ opened #34911
eza-0.23.4       0.23.3 → 0.23.4 · tart:tahoe ✓ · Portfile +3 −3, cargo.crates +2 −2
                 ! upstream's LICENSE file changed; the Portfile's license line may need to follow
? submit? n      · kept for later: dockhand edit eza
fzf-0.66.0       0.65.2 → 0.66.0 · tart:tahoe ✓ · Portfile +3 −3
? submit? y      ✓ opened #34912
```

The `!` lines come from comparing the old and new upstream archives, which dockhand already fetched for the checksums. It looks for changed license files, build files, and declared dependencies. Those are what a reviewer asks about, and what a passing build can't catch.

`server.updates = "submit"` skips that look and opens PRs by itself, at most `submit.daily-limit` a day, and `server status` says plainly when it's on. It's there for a maintainer who wants it, but this design doesn't recommend it. A release that builds can still need a person's eyes before their name goes on the PR: a license change, a new dependency, or a compromised upstream release. With `submit --passing` that look takes seconds, and it's the one part of the job a build can't do.

Related ports share a branch only when you ask for it, with `--into`, `revbump --dependents-of`, or by working inside a branch.
...
`dockhand sync` looks right away. A merged branch's records stay, and `status --all` shows them. Its worktree, local branch, and fork branch are removed, each only if it still holds the commit that was merged. Anything else is kept and reported, never discarded.
...
`dockhand status <branch>` is the full picture of one branch:

```console
$ dockhand status poppler-25.09.0
poppler-25.09.0 · ~/src/macports-branches/poppler-25.09.0 · on master 4c1e2d0 (14 behind)
...
Each key runs the verb it names, with that verb's authority, and verbs that cost minutes or act on GitHub confirm first. `watch <branch>` opens with that branch expanded, and `watch --logs` adds a pane that follows the selected build's log.
...
server · dockhand 1.0.0 · up 3h12m · pid 4711
  providers   tart    tahoe (base, xcode) · sequoia · sonoma · 2 VMs, both busy
              prefix  ~/.dockhand/prefix · 143 ports installed · 1 build running
              github  ada/macports-ports · idle
  queue       6 builds (3 running)
  your ports  212 · checked for new releases daily at 07:00 · new releases are built
              overnight and wait for `submit --passing` (server.updates = "test")
...
```console
$ dockhand queue
server · tart 2 of 2 VMs · prefix 1 of 1 · github idle

  #  BRANCH             ON            STATE                             FOR
  1  poppler-25.09.0    tart:tahoe    ◐ pdf2svg build, 7 of 9 done       you, attached
  2  poppler-25.09.0    tart:sonoma   ◐ inkscape build, 4 of 9 done      you
  3  jq-1.8.1           prefix        ◐ jq build                         you
  4  croc-10.2.5        tart:tahoe    ○ queued                           submit
  5  xan-0.53.0         tart:sonoma   ○ queued (retry after fix)         you
  6  pr-34910           github        ○ queued                           a PR on your ports

$ dockhand queue move 5 --top
$ dockhand queue cancel 6
```

The queue follows three rules:

- **A queue item is a branch at one exact state, for one provider and release.** Ports build inside the item, in dependency order. Each provider has its own capacity: Tart as many VMs as you allow, a prefix one build at a time.
- **The newest state wins.** When a branch changes, its queued items are replaced. Running items finish, and their results still count for every port whose inputs didn't change.
- **You first.** Work you're attached to or asked for comes before work the server started by itself: daily updates, retests, and others' PRs. Otherwise, branches take turns, so one big branch doesn't starve a one-port fix, and a morning's thirty updates don't hold up the fix you're waiting on.

### Providers: where builds run, and what passing means

A branch **passes** when every provider it was tested with passes it, under MacPorts' own rule: lint, build, and install decide, and tests are reported but decide only with `--tests required`. A **provider** is a named way of building. Dockhand has four kinds:

| Kind | Builds in | Clean each time | Releases | What it's for |
| --- | --- | --- | --- | --- |
| `tart` | a fresh clone of a prepared macOS VM image | yes | each release with an image (Apple silicon) | the default when it's set up: isolated, closest to CI, the strongest evidence |
| `prefix` | a MacPorts installation on this Mac | no; what earlier builds installed stays | this Mac's | quick builds while you iterate |
| `github` | MacPorts' CI workflow, in your fork | yes | the workflow's runners (macOS 14, 15, and 26 today) | no Tart; or to see what the PR's CI will see |
| `command` | whatever your command does | as it reports | as configured | anything else |

`--on` names the providers, and optionally releases: `--on tart:sonoma,tahoe`, `--on tart:all`, `--on prefix`, `--on github`. A bare release name means Tart, so `--on sonoma` is `--on tart:sonoma`, and `host` is this Mac's release. `--on tart --on github` asks for both, and the branch passes only when both pass it. `test.on` in the config sets the default, which `init` picks from what's ready: Tart if it has an image, otherwise GitHub. Output leaves out the provider's name when it's the default one, as the journeys above do.

**Providers aren't interchangeable, and dockhand doesn't pretend they are.** Each one says what its result establishes: whether the environment was clean, whether trace mode ran, which tests ran, and on which releases. `status`, the PR body, and the checklist repeat that, and no more. A fallback is something you state in advance, `--on tart --else github`, or `test.else` in the config. When it's used, the result is recorded as GitHub's and says so; it never reads as a Tart pass.

`github` runs the MacPorts CI workflow on your fork before the PR exists: default variants, `port lint` without `--nitpick`, no trace mode, and only the built-in test phase. It's the lighter check, and on a Mac that can't run Tart it's how a branch gets built before it becomes a PR.

#### The `prefix` provider

This is a MacPorts installation on this Mac, used the way the PR template asks: `port -vst install`, then `port test`. For each build, dockhand points it at a full snapshot of the branch's tree.

`dockhand provider setup prefix` builds a prefix for dockhand alone, owned by you, at `~/.dockhand/prefix`, using MacPorts' install without root. It needs no `sudo` and leaves the ports you use every day alone. The cost is that MacPorts' binary archives are built for `/opt/local` only, so the first build compiles every dependency from source. They stay installed afterwards, so later builds are quick. Each result says how many ports were already installed, because that's what makes a prefix build less clean than a VM.

Your everyday `/opt/local` can be a provider too, but only on purpose: `dockhand provider add live --kind prefix --path /opt/local`. It's the fastest, since binary archives apply, but it installs the branch's versions over the ones you use, and it needs root. Dockhand records which versions were active beforehand and reactivates them after the build, and says so when it can't. The server can't answer a `sudo` prompt, so unattended builds there wait for you at a terminal unless you've allowed `port` in sudoers yourself. Dockhand won't make that change for you.
...
[provider.buildbox]
kind = "command"
run = "~/bin/dockhand-build"
releases = ["sequoia"]
capacity = 1
```

Dockhand runs the command with a request file. The request names the branch's commits (as a Git bundle), the ports in build order, the release, the variants, and the test policy. The command streams its log on stdout and writes a result file: each port's lint, build, install, and test outcome, and the environment it ran in. Dockhand records that as the provider's report, labelled with its name in `status` and the PR, since it can't check the command's work itself. This is the way in for any other method.
...
- **Keeps master fresh.** It fetches `master` so that authoring starts from a recent base, and marks branches that no longer rebase cleanly.
- **Keeps your ports current, as far as you let it.** Every morning it checks your ports for new releases. `server.updates` sets how far it goes:
  ...
  - `draft` prepares a branch for each update and stops at the local checks;
  - `test` goes on to build each one with your providers, so that by morning the passing ones wait only for `submit --passing` (the recommended setting);
  - `submit` opens PRs for whatever passes, at most `submit.daily-limit` a day ([§5.12](#512-working-through-your-ports)).
- **Tests continuously, if asked.** With `server.retest = true`, every new state of an open branch is queued for testing, making the server your personal CI.
...
- **Cleans up.** It prunes old VMs, logs, and worktrees on the same rules `gc` applies, and never touches a branch that is still open.

## 8. Reference

### Selectors

Wherever a command takes a branch, dockhand resolves the argument as follows:

1. an exact branch name: `jq-1.8.1`
2. a PR: `#34901`, `34901` for `fetch`, or the PR's URL
3. a port the branch changes, if exactly one open branch changes it: `jq`
4. `.`, or nothing: the current branch

An argument that matches several branches is refused with the list on a script, and offered as a picker on a terminal. Authoring commands take ports, not branches. They add to the current branch if you're in one, and otherwise create one: `--into <branch>` adds elsewhere, and `--new` insists on a fresh branch. If a port already has an open branch elsewhere, dockhand says which one and asks, since two open PRs for one port is usually a mistake.
...
| `--on <provider>[:<releases>]` | Which providers build, and on which releases ([§7](#providers-where-builds-run-and-what-passing-means)). |
| `--else <provider>` | Which provider builds if the first choice can't; the result says which served. |
| `--into <branch>`, `--new` | Where authoring lands. |
...
| `--all` | Include what is hidden by default (retired branches, current ports, finished queue items). It never means "the whole tree"; that's `--everything`, on `outdated` and `info` only. |
...
Flags that belong to one command are described with it: `--together` on `revbump`; `--variants` on `test`; `--markdown`, `--comment`, and `--review` on `check`; and `--as-is`, `--no-test`, `--draft`, `--passing`, `--type`, and `--skip-notification` on `submit`.
...
| Merge commits | ✗ merges in the branch | `rebase` |
...
| Base | ! `master` has moved; ✗ the branch no longer applies to it | `rebase` |
...
- **`###### Type(s)`**: `bugfix`, `enhancement`, and `security fix` are ticked when the branch says so. A CVE identifier in a commit ticks `security fix`, and `--type bugfix` names the others. MacPorts' bot finds "update" and "submission" by itself, from a title containing ": update" and from a new Portfile, so dockhand makes sure the title lets it.
- **`###### Tested on`**: the template's own lines for each release built, `macOS 26.1 25B78 arm64` followed by the Xcode or Command Line Tools version. Each release says which provider built it and what that establishes:
  - `tart`: "built in a clean VM";
  - `prefix`: "built in a MacPorts prefix on this Mac, with 143 ports already installed";
  - `github`: "MacPorts CI workflow in ada/macports-ports", with the run's link;
  - `command`: "reported by buildbox".

  Below them is a grid of port × provider and release, with a pre-existing failure marked as one, and the MacPorts version each ran.
- **`###### Verification`**: each item is ticked from recorded results or left unticked with the reason beside it:
  - `port lint`, `sudo port test`, and the `-vst` install come from the build logs. The install box is ticked only if trace mode really ran. GitHub's workflow never traces, and MacPorts before 2.13 can't trace on arm64 macOS 15 and later. In those cases the box says so rather than being ticked.
...
- **JSON.** Every command writes one envelope, `{"command", "exit_code", "error", "result"}`, with snake_case keys throughout. With `--json`, stderr is JSON lines, `{"time","level","branch","port","message"}`.
...
[update]
then = "stop"              # stop | test | submit: how far a passing branch goes by itself

[test]
on = ["tart:host"]         # or ["tart:tahoe,sonoma"], ["prefix"], ["github"], ["tart", "github"]
...
idle-exit = "15m"
notify = true
retest = false             # test every new state of open branches
updates = "list"           # list | draft | test | submit: what the daily release check does
updates-at = "07:00"

[provider.prefix]
path = "~/.dockhand/prefix"

[provider.buildbox]          # a provider of your own
kind = "command"
run = "~/bin/dockhand-build"
releases = ["sequoia"]
```
...
- Every rewrite of a branch's history keeps the previous head, and `undo` restores it. The last ten are kept per branch.
- `tidy` never changes the final tree, and `save` only ever folds edits into the branch they were made in.
...
| --- | --- | --- |
| Branches across ports; `revbump --dependents-of`; `tidy` | A contribution becomes an ordered list of commits, each scoped to one port directory (or one uniform rebuild across several), with `_resources` allowed; publication and evidence cover every changed port | §3.1 |
...
| Four provider kinds; "passes" across several | Today's provider contract stays. `prefix` and `command` join `tart` and `github`, and each provider declares what its evidence establishes (clean environment, trace mode, tests, releases). The PR body, the checklist, and `then = "submit"` read that declaration | extends today's providers |
| `submit --passing` upstream notes | Compare the old and new upstream archives for changed license files, build files, and declared dependencies | new |
| Worktrees per branch | Sparse worktrees, and the traced-read guarantee that makes a sparse tree safe to evaluate | §3.7 |
| Build in dependency order from the branch's tree | One VM per platform builds a branch's ports in dependency order, as MacPorts CI does | new |
| "caused by this change" vs "pre-existing" | Baseline builds at the branch's base, recorded as their own results | new, in the spirit of the failure-attribution principle |
...
| `verify` | `test` | Builds go through the queue; `--on <provider>:<releases>` replaces `--provider`, `--image`, and `--os`. |
...
| `rebase` | `rebase` | Works in the branch's own worktree; no switching away. |
...
| `setup` | `init`, `provider setup tart` | |
| `reassociate` | `rename`, or automatic | Dockhand follows a branch renamed with Git. |
| `gc`, `db` | `gc`, `db` | |
...
1. **The server stays on your own Mac.** Dockhand is for an individual maintainer working through the ports they maintain, so the server serves one person on one machine. The first draft's remote build servers are removed, and a shared service for several contributors is out of scope. Testing others' PRs on your ports stays, as an opt-in feature of the same server. Building elsewhere remains possible through a provider (`github`, or a `command`), which is a different thing from where the server runs.
2. **Branches live in sparse worktrees**, in a visible directory beside the ports clone, so that they're easy to open, browse, and edit directly ([§3](#where-branches-live)).
3. **`update` stops after the local checks** by default. A maintainer who wants whatever passes to go straight to a PR sets `update.then = "submit"`, or passes `--submit` for one run ([§5.12](#512-working-through-your-ports)).
...
5. **Passing is a provider's verdict, and providers stay.** Something that builds the branch decides whether it passes. There are four kinds: a Tart VM, a MacPorts prefix on this Mac, GitHub Actions in your fork, or a command of your own. A branch passes when every provider in `test.on` passes it ([§7](#providers-where-builds-run-and-what-passing-means)).
6. **The screen says "branch".** The design's term stays *changeset*, and the engine can keep it. On screen it's a branch, for two reasons. A branch is what contributors create, push, and see as the head of their PR, so there's no new word to learn. And in MacPorts, "changeset" already means something else: the guide tells contributors to link a single commit from a Trac ticket as `[changeset:<hash>/macports-ports …]`, which Trac serves at `/changeset/<hash>/macports-ports`, and Mercurial uses the word the same way. A long-time contributor would read "changeset" as one commit, the opposite of what it means here. The price is small: dockhand shows only the branches it tracks, and says "tracked" where the difference matters.

### Where this draft pushes back

These are places where the design goes a different way from what was asked, or from what's easiest to build.

1. **No unattended PRs by default.** Whatever passes can go straight to a PR when you start the run (decision 3). For the server's own daily run, this design recommends `server.updates = "test"`, followed by `submit --passing` in the morning, over `"submit"`. A build can't see a license change, a new dependency, or a compromised release, and the look that catches them takes seconds. `"submit"` remains available ([§5.12](#512-working-through-your-ports)).
2. **Your everyday `/opt/local` is not the default prefix.** It's the fastest provider, and literally what the PR template asks for, but it installs untested versions over the ports you use and needs root, which an unattended server can't supply safely. The default `prefix` is a separate, user-owned installation, and `/opt/local` is an explicit opt-in ([§7](#the-prefix-provider)).
3. **Four provider kinds and a command, not a plugin API.** "Any other number of methods" is covered by the `command` kind, with a request file in and a result file out. A plugin interface would be a larger compatibility promise, made before anyone has asked for a fifth kind.
4. **One provider never passes for another.** A pass on a prefix doesn't establish what a pass in a clean VM does, and GitHub's workflow doesn't trace. So "passes" always names its providers, a fallback is recorded as what actually ran, and the PR says which provider built what.

### Still open

Nothing from these review rounds. The next step is the engine design that [§9](#9-what-this-asks-of-the-engine) outlines.
