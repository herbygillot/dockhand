...
Run `make` (or `make build`) to build `./dockhand`. Use `make test`, `make test-race`, `make vet`, and `make fmt-check` (which names any Go file outside `vendor` that `gofmt` would change; CI runs it) for checks, and `make clean` to remove the binary. Override the output with `make BINARY=/path/to/dockhand` or the Go executable with `make GO=/path/to/go`.
