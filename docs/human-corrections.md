...
A contributor can switch to a Dockhand branch, edit Portfiles or patches, and use `git commit --amend`. No marker in Git config, commit trailers, private refs, or tags is required. `verify` captures tracked working-tree changes and staged new files; `verify --adopt NAME` selects committed contents; `--branch` selects a tracked contribution branch. GitHub verification requires committed contents and an explicit branch. Publication always selects committed contents.
...
Interface: `dockhand amend [target] [--branch NAME] [--dry-run] [--to branch|verified|pr] [--unverified] [--detach|--trace]`.

The default branch is the current tracked contribution. For this first implementation, stage the intended contents before adopting a checked-out amendment; Dockhand does not automatically stage edits. A target, or `--branch`, selects the contribution. Amendment captures tracked edits and staged new files using the same capture boundary as verification, preserves the original contribution base, and constructs one replacement contribution commit. It preserves the title unless the user explicitly supplies a replacement. Changes outside the expected port directory or ambiguous untracked files require the user to resolve them first. It does not absorb unrelated commits or silently stage new files.

`--dry-run` shows the difference from the current recorded revision without moving refs or accepting work. An accepted amendment follows preparation, guarded branch replacement, verification, and publication of the updated PR, in the foreground. With `--to verified`, it stops after verification; `--unverified` updates the PR without a build and says so in its body, and with both it stops at the replaced branch; `--detach` returns once it is accepted. It uses the existing state/claim lifecycle rather than spawning a separate controller.

Updating a checked-out branch requires one of two states of the checkout, rechecked under the index lock against their exact captured identities. Either the index and working tree already hold the amendment, which is the amend a person staged; or they are clean at the previous commit, which is every amendment dockhand prepares onto a contribution, and then the checkout moves forward with the branch the way a fast-forward would: the index and working tree take the candidate's tree first, touching only the files it changes, and the ref moves after, so an interruption leaves the amendment staged and the retry accepts it under the first state. Any other checkout is the person's work: preserve the candidate and report the conflict; never reset or discard it. A branch checked out in a different worktree must be handled explicitly rather than invalidating that worktree's index.

## Rebase and squash

Interface: `dockhand rebase [target] [--branch NAME] [--dry-run] [--to branch|verified|pr] [--unverified] [--detach|--trace]`.
...
The implementation preserves the recorded contribution message unless `--subject` supplies what follows the port name in its subject or `--closes` and `--see` add ticket trailers it does not already carry; a rewrite that keeps the tree reuses its evidence. It requires staged matching contents for a checked-out amendment, or a checkout clean at the previous commit, which is moved forward with the branch: the index and working tree take the candidate's tree first, so an interruption leaves the amendment staged and the retry accepts it, and refuses a checkout-changing rebase or a branch occupied by another worktree. These conservative limits replace the proposed automatic checkout adoption until that can preserve both index and worktree intent across crashes. Rebase preparation happens before durable job acceptance; a conflict reports its retained worktree directly. Accepted candidates use the ordinary durable preparation/integration checkpoints.
