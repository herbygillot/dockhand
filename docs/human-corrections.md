...
The implementation preserves the recorded contribution message unless `--subject` supplies what follows the port name in its subject or `--closes` and `--see` add ticket trailers it does not already carry; a rewrite that keeps the tree reuses its evidence. It requires staged matching contents for a checked-out amendment and refuses a checkout-changing rebase or a branch occupied by another worktree. These conservative limits replace the proposed automatic checkout adoption until that can preserve both index and worktree intent across crashes. Rebase preparation happens before durable job acceptance; a conflict reports its retained worktree directly. Accepted candidates use the ordinary durable preparation/integration checkpoints.
