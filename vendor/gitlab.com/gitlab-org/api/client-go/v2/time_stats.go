...
var (
	...
	routeProjectsIDIDIDResetSpentTime    = route("projects/%s/%s/%d/reset_spent_time")
	routeProjectsIDIDIDResetTimeEstimate = route("projects/%s/%s/%d/reset_time_estimate")
	routeProjectsIDIDIDTimeEstimate      = route("projects/%s/%s/%d/time_estimate")
	routeProjectsIDIDIDTimeStats         = route("projects/%s/%s/%d/time_stats")
)
...
func (s *timeStatsService) setTimeEstimate(pid any, entity string, issue int64, opt *SetTimeEstimateOptions, options ...RequestOptionFunc) (*TimeStats, *Response, error) {
	return do[*TimeStats](s.client,
		...
		withPath(routeProjectsIDIDIDTimeEstimate, ProjectID{pid}, entity, issue),
		withAPIOpts(opt),
...
func (s *timeStatsService) resetTimeEstimate(pid any, entity string, issue int64, options ...RequestOptionFunc) (*TimeStats, *Response, error) {
	return do[*TimeStats](s.client,
		withMethod(http.MethodPost),
		withPath(routeProjectsIDIDIDResetTimeEstimate, ProjectID{pid}, entity, issue),
		withRequestOpts(options...),
	)
}
...
func (s *timeStatsService) resetSpentTime(pid any, entity string, issue int64, options ...RequestOptionFunc) (*TimeStats, *Response, error) {
	return do[*TimeStats](s.client,
		withMethod(http.MethodPost),
		withPath(routeProjectsIDIDIDResetSpentTime, ProjectID{pid}, entity, issue),
		withRequestOpts(options...),
	)
}
...
func (s *timeStatsService) getTimeSpent(pid any, entity string, issue int64, options ...RequestOptionFunc) (*TimeStats, *Response, error) {
	return do[*TimeStats](s.client,
		withPath(routeProjectsIDIDIDTimeStats, ProjectID{pid}, entity, issue),
		withRequestOpts(options...),
	)
}
