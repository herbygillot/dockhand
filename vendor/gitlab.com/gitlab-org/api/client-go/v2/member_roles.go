...
var (
	routeGroupsIDMemberRoles   = route("groups/%s/member_roles")
	routeGroupsIDMemberRolesID = route("groups/%s/member_roles/%d")
	routeMemberRoles           = route("member_roles")
	routeMemberRolesID         = route("member_roles/%d")
)
...
func (s *MemberRolesService) DeleteInstanceMemberRole(memberRoleID int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeMemberRolesID, memberRoleID),
		withAPIOpts(nil),
		withRequestOpts(options...),
	)
	return resp, err
...
func (s *MemberRolesService) ListMemberRoles(gid any, options ...RequestOptionFunc) ([]*MemberRole, *Response, error) {
	return do[[]*MemberRole](s.client,
		withMethod(http.MethodGet),
		withPath(routeGroupsIDMemberRoles, GroupID{gid}),
		withAPIOpts(nil),
		withRequestOpts(options...),
	)
}
...
func (s *MemberRolesService) DeleteMemberRole(gid any, memberRole int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		withMethod(http.MethodDelete),
		withPath(routeGroupsIDMemberRolesID, GroupID{gid}, memberRole),
		withAPIOpts(nil),
		withRequestOpts(options...),
	)
	return resp, err
