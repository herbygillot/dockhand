...
var (
	routeGroupsIDEnterpriseUsers                   = route("groups/%s/enterprise_users")
	routeGroupsIDEnterpriseUsersID                 = route("groups/%s/enterprise_users/%d")
	routeGroupsIDEnterpriseUsersIDDisableTwoFactor = route("groups/%s/enterprise_users/%d/disable_two_factor")
)
...
func (s *EnterpriseUsersService) ListEnterpriseUsers(gid any, opt *ListEnterpriseUsersOptions, options ...RequestOptionFunc) ([]*User, *Response, error) {
	return do[[]*User](s.client,
		withPath(routeGroupsIDEnterpriseUsers, GroupID{gid}),
		withAPIOpts(opt),
...
func (s *EnterpriseUsersService) GetEnterpriseUser(gid any, uid int64, options ...RequestOptionFunc) (*User, *Response, error) {
	return do[*User](s.client,
		withPath(routeGroupsIDEnterpriseUsersID, GroupID{gid}, uid),
		withRequestOpts(options...),
	)
}
...
func (s *EnterpriseUsersService) Disable2FAForEnterpriseUser(gid any, uid int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeGroupsIDEnterpriseUsersIDDisableTwoFactor, GroupID{gid}, uid),
		withRequestOpts(options...),
	)
	return resp, err
}
