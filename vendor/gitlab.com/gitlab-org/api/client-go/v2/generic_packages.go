...
var routeProjectsIDPackagesGenericIDIDID = route("projects/%s/packages/generic/%s/%s/%s")
...
func (s *GenericPackagesService) DownloadPackageFile(pid any, packageName, packageVersion, fileName string, options ...RequestOptionFunc) ([]byte, *Response, error) {
	buf, resp, err := do[bytes.Buffer](s.client,
		withPath(routeProjectsIDPackagesGenericIDIDID, ProjectID{pid}, packageName, packageVersion, FileName{fileName}),
