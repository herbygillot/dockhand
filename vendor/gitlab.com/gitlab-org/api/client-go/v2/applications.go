...
var (
	routeApplications              = route("applications")
	routeApplicationsID            = route("applications/%d")
	routeApplicationsIDRenewSecret = route("applications/%d/renew-secret")
)
...
func (s *ApplicationsService) DeleteApplication(application int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeApplicationsID, application),
		withRequestOpts(options...),
	)
	return resp, err
}
...
func (s *ApplicationsService) RenewApplicationSecret(application int64, options ...RequestOptionFunc) (*Application, *Response, error) {
	return do[*Application](s.client,
		withMethod(http.MethodPost),
		withPath(routeApplicationsIDRenewSecret, application),
		withRequestOpts(options...),
	)
}
