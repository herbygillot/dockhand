...
var (
	routeProjectsIDTerraformStateID           = route("projects/%s/terraform/state/%s")
	routeProjectsIDTerraformStateIDLock       = route("projects/%s/terraform/state/%s/lock")
	routeProjectsIDTerraformStateIDVersionsID = route("projects/%s/terraform/state/%s/versions/%d")
)
...
func (s *TerraformStatesService) DownloadLatest(pid any, name string, options ...RequestOptionFunc) (io.ReadCloser, *Response, error) {
	r, resp, err := do[bodyReader](s.client,
		withPath(routeProjectsIDTerraformStateID, ProjectID{pid}, name),
...
func (s *TerraformStatesService) Download(pid any, name string, serial uint64, options ...RequestOptionFunc) (io.ReadCloser, *Response, error) {
	r, resp, err := do[bodyReader](s.client,
		withPath(routeProjectsIDTerraformStateIDVersionsID, ProjectID{pid}, name, serial),
		withRequestOpts(options...),
	)
	if err != nil {
		return nil, resp, err
...
func (s *TerraformStatesService) Lock(pid any, name string, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeProjectsIDTerraformStateIDLock, ProjectID{pid}, name),
		withRequestOpts(options...),
	)
	return resp, err
}
