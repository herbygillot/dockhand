...
var (
	routeTodos             = route("todos")
	routeTodosIDMarkAsDone = route("todos/%d/mark_as_done")
	routeTodosMarkAsDone   = route("todos/mark_as_done")
)
...
func (s *TodosService) MarkTodoAsDone(id int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeTodosIDMarkAsDone, id),
		withRequestOpts(options...),
	)
	return resp, err
}
