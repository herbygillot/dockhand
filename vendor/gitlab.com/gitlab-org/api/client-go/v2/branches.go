...
var (
	routeProjectsIDRepositoryBranches       = route("projects/%s/repository/branches")
	routeProjectsIDRepositoryBranchesID     = route("projects/%s/repository/branches/%s")
	routeProjectsIDRepositoryMergedBranches = route("projects/%s/repository/merged_branches")
)
...
func (s *BranchesService) ListBranches(pid any, opts *ListBranchesOptions, options ...RequestOptionFunc) ([]*Branch, *Response, error) {
	return do[[]*Branch](s.client,
		...
		withPath(routeProjectsIDRepositoryBranches, ProjectID{pid}),
		withAPIOpts(opts),
...
func (s *BranchesService) GetBranch(pid any, branch string, options ...RequestOptionFunc) (*Branch, *Response, error) {
	return do[*Branch](s.client,
		withMethod(http.MethodGet),
		withPath(routeProjectsIDRepositoryBranchesID, ProjectID{pid}, branch),
		withRequestOpts(options...),
	)
}
...
func (s *BranchesService) DeleteMergedBranches(pid any, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		withMethod(http.MethodDelete),
		withPath(routeProjectsIDRepositoryMergedBranches, ProjectID{pid}),
		withRequestOpts(options...),
	)
	return resp, err
}
