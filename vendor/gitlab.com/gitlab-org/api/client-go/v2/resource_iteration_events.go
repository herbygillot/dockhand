...
var (
	routeProjectsIDIssuesIDResourceIterationEvents   = route("projects/%s/issues/%d/resource_iteration_events")
	routeProjectsIDIssuesIDResourceIterationEventsID = route("projects/%s/issues/%d/resource_iteration_events/%d")
)
...
func (s *ResourceIterationEventsService) ListIssueIterationEvents(pid any, issue int64, opt *ListIterationEventsOptions, options ...RequestOptionFunc) ([]*IterationEvent, *Response, error) {
	return do[[]*IterationEvent](s.client,
		withPath(routeProjectsIDIssuesIDResourceIterationEvents, ProjectID{pid}, issue),
		withAPIOpts(opt),
...
func (s *ResourceIterationEventsService) GetIssueIterationEvent(pid any, issue int64, event int64, options ...RequestOptionFunc) (*IterationEvent, *Response, error) {
	return do[*IterationEvent](s.client,
		withPath(routeProjectsIDIssuesIDResourceIterationEventsID, ProjectID{pid}, issue, event),
		withRequestOpts(options...),
	)
}
