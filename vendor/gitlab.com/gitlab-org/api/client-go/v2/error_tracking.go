...
var (
	routeProjectsIDErrorTrackingClientKeys   = route("projects/%s/error_tracking/client_keys")
	routeProjectsIDErrorTrackingClientKeysID = route("projects/%s/error_tracking/client_keys/%d")
	routeProjectsIDErrorTrackingSettings     = route("projects/%s/error_tracking/settings")
)
...
func (s *ErrorTrackingService) GetErrorTrackingSettings(pid any, options ...RequestOptionFunc) (*ErrorTrackingSettings, *Response, error) {
	return do[*ErrorTrackingSettings](s.client,
		withPath(routeProjectsIDErrorTrackingSettings, ProjectID{pid}),
...
func (s *ErrorTrackingService) ListClientKeys(pid any, opt *ListClientKeysOptions, options ...RequestOptionFunc) ([]*ErrorTrackingClientKey, *Response, error) {
	return do[[]*ErrorTrackingClientKey](s.client,
		withPath(routeProjectsIDErrorTrackingClientKeys, ProjectID{pid}),
		withAPIOpts(opt),
		withRequestOpts(options...),
	)
}
...
func (s *ErrorTrackingService) DeleteClientKey(pid any, keyID int64, options ...RequestOptionFunc) (*Response, error) {
	_, resp, err := do[none](s.client,
		...
		withPath(routeProjectsIDErrorTrackingClientKeysID, ProjectID{pid}, keyID),
		withRequestOpts(options...),
	)
	return resp, err
}
