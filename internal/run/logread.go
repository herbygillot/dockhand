...
// dockhand ever learns about a build beyond pass or fail, and these
...
// return becomes the record, so their answers are as much a wire format
// as the record's fields are.
//
...
// BlockedDetail names the dependency that blocked a verification.
//
// IT NO LONGER SAYS WHETHER ANYONE MAINTAINS THAT PORT, and the loss is
// deliberate rather than an oversight. Whether a blamed dependency
// declares nomaintainer is a fact about the ports TREE — the shipped
// settlement answered it with a filesystem glob run between two halves
// of one judgment and fed the answer back in — and Evidence carries no
// such field, because the one interpreter of provider evidence does not
// read the tree. The names travel out on Judgment.Blamed instead, and
// the caller that may look annotates its own report.
func BlockedDetail(dep string) string {
	return fmt.Sprintf("dependency %s fails to build; the change itself is untested", dep)
}
...
// needsLog reports whether settling this job requires its guest log,
// which is a question the observer must ask before judging because
// fetching one is a round trip to the guest.
//
// A failure always needs its log: the diagnosis, the refusal and the
// blame all come out of it. A PASS NEEDS ONE TOO, and that is a
// simplification the shipped tree could not make: it asked
// verdict.NeedsLog(state, linted) because record.Run carried a Linted
// bool beside a Lint string, so a run that had not linted had nothing to
// corroborate. record.Run now spells that as one field (Lint is nil when
// no lint ran), and the runner it is asked of lints unconditionally —
// measured in internal/verify/tart, where soloArgvFiles and
// cohortArgvFiles both write an argv.lint for every member and only the
// TEST argv is conditional. So the second parameter had exactly one
// value at every call site, and a passing log is read for the lint line
// BEFORE its worker is released, because releasing it puts the log out
// of reach.
//
// A job still running and an environment that never came up need
// nothing: there is no diagnosis in the first and no output in the
// second.
func needsLog(state verify.State) bool {
	//nolint:exhaustive // the two states that need no log are named; a state from another shape is read as needing one, which costs a fetch and never a wrong verdict
	switch state {
