...
func (t *transaction) Attempt(ctx context.Context, id record.AttemptID) (record.Attempt, error) {
	...
	var created int64
	err := t.conn.QueryRowContext(ctx, `SELECT id,job_id,target_id,coalesce(revision_id,''),source_id,build,state,claim_owner,claim_generation,claim_until,retry_at,cancel_sent_at,cancel_observe,last_error,created_at,consecutive_failures FROM attempts WHERE repository_id=? AND id=?`, t.repo, id).Scan(&v.ID, &v.JobID, &v.TargetID, &v.Spec.RevisionID, &source, &raw, &v.State, &owner, &v.ClaimGeneration, &claimUntil, &retry, &cancel, &v.CancelPendingObservation, &v.LastError, &created, &v.ConsecutiveFailures)
	if err != nil {
...
func (t *transaction) PutAttempt(ctx context.Context, v record.Attempt) error {
	...
	if found {
		err = t.exec(ctx, `UPDATE attempts SET state=?,claim_owner=?,claim_generation=?,claim_until=?,consecutive_failures=?,retry_at=?,next_action_at=?,cancel_sent_at=?,cancel_observe=?,last_error=? WHERE repository_id=? AND id=?`, v.State, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, nullableTime(v.RetryAt), next, nullableTime(v.CancelSentAt), v.CancelPendingObservation, v.LastError, t.repo, v.ID)
	} else {
		...
		err = t.exec(ctx, `INSERT INTO attempts(id,repository_id,job_id,target_id,revision_id,source_id,build,state,claim_owner,claim_generation,claim_until,consecutive_failures,retry_at,next_action_at,cancel_sent_at,cancel_observe,last_error,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, v.ID, t.repo, v.JobID, v.TargetID, nullableID(v.Spec.RevisionID), source, raw, v.State, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, nullableTime(v.RetryAt), next, nullableTime(v.CancelSentAt), v.CancelPendingObservation, v.LastError, v.CreatedAt.UnixMilli())
	}
	if err != nil {
		return err
	}
...
func (t *transaction) Resource(ctx context.Context, id record.ResourceID) (record.Resource, error) {
	...
	err := t.conn.QueryRowContext(ctx, `SELECT id,attempt_id,submission_id,provider,handle,state,claim_owner,claim_generation,claim_until,retry_at,retain_until,released_at,artifacts_pruned_at,last_error,consecutive_failures FROM resources WHERE repository_id=? AND id=?`, t.repo, id).Scan(&v.ID, &v.AttemptID, &v.SubmissionID, &v.Handle.Provider, &v.Handle.ID, &v.State, &owner, &v.ClaimGeneration, &until, &retry, &retain, &released, &pruned, &v.LastError, &v.ConsecutiveFailures)
...
func (t *transaction) PutResource(ctx context.Context, v record.Resource) error {
	...
	if found {
		return t.exec(ctx, `UPDATE resources SET state=?,claim_owner=?,claim_generation=?,claim_until=?,consecutive_failures=?,retry_at=?,next_action_at=?,retain_until=?,released_at=?,last_error=? WHERE repository_id=? AND id=?`, v.State, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, nullableTime(v.RetryAt), next, nullableTime(v.RetainUntil), nullableTime(v.ReleasedAt), v.LastError, t.repo, v.ID)
	}
	return t.exec(ctx, `INSERT INTO resources(id,repository_id,attempt_id,submission_id,provider,handle,state,claim_owner,claim_generation,claim_until,consecutive_failures,retry_at,next_action_at,retain_until,released_at,last_error) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, v.ID, t.repo, v.AttemptID, v.SubmissionID, v.Handle.Provider, v.Handle.ID, v.State, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, nullableTime(v.RetryAt), next, nullableTime(v.RetainUntil), nullableTime(v.ReleasedAt), v.LastError)
}
