...
func (t *transaction) Job(ctx context.Context, id record.JobID) (record.Job, error) {
	...
	err := t.conn.QueryRowContext(ctx, `SELECT id,request_id,change_id,spec_change_id,input_revision,result_revision,source_id,action,phase,destination,verification,options,state,accepted_at,cancel_at,admitted_at,finished_at,detail,claim_owner,claim_generation,claim_until,retry_at,prepared,resolved_release,reused_attempt,reuse_detail,consecutive_failures,consecutive_waits FROM jobs WHERE repository_id=? AND id=?`, t.repo, id).Scan(&v.ID, &v.RequestID, &change, &specChange, &input, &result, &source, &v.Spec.Action, &v.Phase, &v.Spec.Destination, &v.Spec.Verification, &raw, &v.State, &accepted, &canceled, &admitted, &finished, &v.Detail, &owner, &v.ClaimGeneration, &until, &retry, &prepared, &release, &reused, &v.ReuseDetail, &v.ConsecutiveFailures, &v.ConsecutiveWaits)
	if err != nil {
		return v, storageError(err)
	}
...
func (t *transaction) PutJob(ctx context.Context, v record.Job) error {
	...
	if err == nil {
		...
		if err = t.exec(ctx, "UPDATE jobs SET change_id=?,result_revision=?,phase=?,state=?,cancel_at=?,admitted_at=?,finished_at=?,detail=?,claim_owner=?,claim_generation=?,claim_until=?,consecutive_failures=?,consecutive_waits=?,retry_at=?,prepared=?,resolved_release=?,reused_attempt=?,reuse_detail=? WHERE repository_id=? AND id=?", nullableID(v.ChangeID), nullableID(v.ResultRevision), v.Phase, v.State, nullableTime(v.CancelRequestedAt), nullableTime(v.AdmittedAt), nullableTime(v.FinishedAt), v.Detail, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, v.ConsecutiveWaits, nullableTime(v.RetryAt), prepared, release, nullableID(v.ReusedAttempt), v.ReuseDetail, t.repo, v.ID); err != nil {
			return err
		}
	...
	err = t.exec(ctx, `INSERT INTO jobs(id,repository_id,request_id,change_id,spec_change_id,input_revision,result_revision,source_id,action,phase,destination,verification,options,state,accepted_at,cancel_at,admitted_at,finished_at,detail,claim_owner,claim_generation,claim_until,consecutive_failures,consecutive_waits,retry_at,prepared,resolved_release,reused_attempt,reuse_detail) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, v.ID, t.repo, v.RequestID, nullableID(v.ChangeID), nullableID(v.Spec.ChangeID), nullableID(v.Spec.InputRevision), nullableID(v.ResultRevision), source, v.Spec.Action, v.Phase, v.Spec.Destination, v.Spec.Verification, raw, v.State, v.AcceptedAt.UnixMilli(), nullableTime(v.CancelRequestedAt), nullableTime(v.AdmittedAt), nullableTime(v.FinishedAt), v.Detail, owner, v.ClaimGeneration, until, v.ConsecutiveFailures, v.ConsecutiveWaits, nullableTime(v.RetryAt), prepared, release, nullableID(v.ReusedAttempt), v.ReuseDetail)
	if err != nil {
		return err
	}
	return t.scheduleJob(ctx, v.ID)
