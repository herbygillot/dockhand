...
func (t *transaction) VerificationCandidates(ctx context.Context, query state.VerificationQuery) ([]record.Attempt, error) {
	...
	if query.Target.Name == "" || query.Target.Portfile == "" || query.Limit < 1 || query.Limit > 32 || (query.Tree != "" && !objectID(query.Tree)) {
...
const verificationTreeSQL = `SELECT a.id FROM sources AS s INDEXED BY sources_reuse_tree CROSS JOIN attempts AS a INDEXED BY attempts_reuse_source
   ...
   AND json_extract(a.build,'$.Target.Name')=? AND json_extract(a.build,'$.Target.Portfile')=?
