...
func (r *runtime) gcCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Clean up terminal work in the current repository, or in every registered repository with --all-repositories, which also reaches registrations whose checkout no longer exists and could otherwise never release their environments. Release retained environments whose jobs finished before the age threshold; prune diagnostic files only when confirmed release is also that old. Future explicit retention deadlines are honored. History, evidence, submission identities, and lockfiles remain. Active or unresolved attempts are never cleaned up. Old local GitHub log caches are removed only for terminal jobs, and one a download holds is reported as kept for the next sweep; remote logs and database evidence remain. Local branches of merged contributions that were kept at the time, checked out or busy, are deleted once they are free and still hold the published commit; a branch that moved is reported and kept. Shared PortIndex caches are removed by last-use age, across repositories, under their existing locks. Busy caches are skipped. This command does not advance jobs.",
		RunE: func(cmd *cobra.Command, _ []string) error {
