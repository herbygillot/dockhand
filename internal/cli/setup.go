...
func (r *runtime) setupCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long:        "Check the native MacPorts platform and prepare a clean local Tart image with the guest agent, command line tools, and MacPorts. An existing image is validated in a disposable clone. Missing images are provisioned from the matching vanilla macOS image.",
		Args:        cobra.NoArgs,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			_, err = fmt.Fprintf(cmd.OutOrStdout(), "%s verification image %s (%s %s %s, MacPorts %s, guest agent %s).\n", verb, plain(result.Image), plain(result.Platform.OS), plain(result.Platform.Version), plain(result.Platform.Architecture), plain(result.MacPortsVersion), plain(result.GuestAgentVersion))
			return err
		},
	}
