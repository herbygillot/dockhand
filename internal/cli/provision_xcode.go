...
func (a provisionXcodeAction) Execute(ctx context.Context, s *Services) error {
	releases, err := a.targets(ctx, s.Tools)
	if err != nil {
		return err
	}
	fmt.Fprintln(s.Out, "Xcode provisioning — what each release needs:")
	fmt.Fprintln(s.Out)
	for _, r := range releases {
		version, capped := xcode.Recommended(r)
		...
		fmt.Fprintf(s.Out, "  %-10s needs %s\n", r.Name, want)
		if a.from != "" {
			switch path, v, err := provision.PickXcode(a.from, r); {
			case err != nil:
				fmt.Fprintf(s.Out, "  %-10s missing: place %s in %s\n", "", archive, a.from)
			case capped && v != version:
				// Usable is not ready: an older archive provisions, but
				// the release could carry more.
				fmt.Fprintf(s.Out, "  %-10s usable: %s (Xcode %s — below the recommended %s)\n", "", path, v, version)
			default:
				fmt.Fprintf(s.Out, "  %-10s ready: %s (Xcode %s)\n", "", path, v)
			}
		}
	}
	dir := a.from
...
// provisionXcode builds `provision tart xcode`, NESTED under the tart
// provisioner: the verb performs the Tart-specific installation of a
// toolchain, and which toolchain a Darwin release expects is
// internal/darwin/xcode's — a fact about the OS rather than about a
// provider, moved there precisely so a second provider could reach it.
func provisionXcode(s *Services) *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// A guide reads the machine and explains it. Nothing else.
			if err := s.Acquire(cmd.Context(), app.Needs{}); err != nil {
				return err
			}
			return provisionXcodeAction{release: release, from: from}.Execute(cmd.Context(), s)
		},
	}
