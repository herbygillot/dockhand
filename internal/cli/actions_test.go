...
func TestVerificationArgumentsFailBeforeOpeningState(t *testing.T) {
	for _, args := range [][]string{{"verify", "jq"}, {"verify", "jq", "--image", "base", "--capacity", "0"}, {"verify", "jq", "--image", "base", "--tests", "perhaps"}, {"verify", "jq", "--image", "base", "--variant", "ssl"}, {"verify", "jq", "--image", "base", "--variant", "+ssl", "--variant=-ssl"}} {
