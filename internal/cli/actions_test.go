...
func TestJSONResultKeepsLogsAndProgressOffStdout(t *testing.T) {
	...
	reporter := newReporter(&stderr, nil, false, progress.Verbose, false, false)
...
func TestTraceResumesOffsetsAndDrainsTerminalLogs(t *testing.T) {
	...
	reporter := newReporter(&output, provider, true, progress.Verbose, false, false)
...
func TestCompletionEmphasizesPassedVerificationAndKeepsReuseDecisionEarlier(t *testing.T) {
	t.Parallel()
	var output bytes.Buffer
	reporter := newReporter(&output, nil, false, progress.Verbose, false, false)
