...
`, server.URL, strings.Repeat("0", 64), server.URL)
	...
	out.Reset()
	logs.Reset()
	require.NoError(t, Run(t.Context(), []string{"bump", "fixture-1.16", "--to", "branch", "--json"}, Streams{Out: &out, Err: &logs}, config), logs.String())
	...
	out.Reset()
	logs.Reset()
	require.NoError(t, Run(t.Context(), []string{"bump", "fixture-1.16", "1.16.2", "--to", "branch", "--json"}, Streams{Out: &out, Err: &logs}, config), logs.String())
	decodeResult(t, out.Bytes(), &result)
