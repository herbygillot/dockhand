...
// run performs the report. It declares a TREE and nothing else: no
// repository, no state ref, no verifier, no forge runner of its own —
// the forge questions go through the observer's own seam under
// courtesy's pacing, and the fetch session is opened lazily and only
// where a livecheck is actually reached.
func (a outdatedAction) run(ctx context.Context, s *Services) error {
	if err := s.Acquire(ctx, app.Needs{Tree: true}); err != nil {
		return err
	}
	targets, err := resolveTargets(ctx, s, a.all, a.args)
	if err != nil {
		return err
	}
	...
	obs, cache, err := a.observer(ctx, s)
	if err != nil {
		return err
	}
	...
	emit := func(r upstream.Row) {
		...
		case a.asJSON:
			err = rows.Encode(r)
		case a.show(r):
			_, err = fmt.Fprintln(s.Out, textRow(r))
		}
	...
	fmt.Fprint(s.Err, census.String())
	if n := cache.Prune(pruneAge); n > 0 {
		fmt.Fprintf(s.Err, "  %-12s %5d removed\n", "cache", n)
	}
	if writeErr != nil {
...
//
// The lock is not decoration. The composition root's facilities are
// resolved before an operation begins and are not synchronized, and
// this resolves one DURING the sweep, from whichever worker arrives
// first.
//
...
func (l *lazyLivecheck) Livecheck(ctx context.Context, portdir, subport string) (portfetch.LivecheckResult, error) {
	...
	if l.f == nil && l.err == nil {
		l.f, l.err = l.s.fetchSession(l.run)
	}
	f, err := l.f, l.err
	l.mu.Unlock()
	if err != nil {
...
func (a outdatedAction) observer(ctx context.Context, s *Services) (*upstream.Observer, *courtesy.Cache, error) {
	m, cache := manners(s, a.noCache, a.ttl, a.pace)
	return &upstream.Observer{
		Tools:   s.Tools,
		Gh:      upstream.GhRunner(s.Forge),
		Fetcher: &lazyLivecheck{run: ctx, s: s},
		Pacer:   m.Pacer,
		Cache:   m.Cache,
		Agent:   m.Agent,
		Deep:    a.deep,
...
func manners(s *Services, noCache bool, ttl, pace time.Duration) (upstream.Manners, *courtesy.Cache) {
	dir := ""
	if !noCache {
	...
	return upstream.Manners{
		...
		Cache: cache,
		Agent: courtesy.UserAgent(s.Version),
	}, cache
}
...
// branch exists.
func sweepManners(s *Services) (upstream.Manners, *courtesy.Cache) {
	return manners(s, false, defaultTTL, 0)
}
...
// outdatedCmd builds the outdated subcommand.
func outdatedCmd(s *Services) *cobra.Command {
