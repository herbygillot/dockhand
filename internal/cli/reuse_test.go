...
func TestVerifyCLIReusesEvidenceAndFreshFlagIsDurable(t *testing.T) {
	...
	bound, err := services.BindVerification(t.Context(), app.Verification{ID: "original", Branch: "candidate", Selection: macports.Selection{Selector: "fixture"}, Tests: record.TestDeclared})
	require.NoError(t, err)
	...
	require.NoError(t, services.Workflow.State.Update(t.Context(), services.Workflow.Repository, func(ctx context.Context, tx state.Tx) error {
		...
		plan, build, err := verify.PlanSingle(job, record.Revision{})
		if err != nil {
			return err
		}
	...
	require.Contains(t, stderr.String(), "Reused passing verification")
	stdout.Reset()
	stderr.Reset()
