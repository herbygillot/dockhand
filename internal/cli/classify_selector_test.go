...
func TestClassifyAllErrorOrderIsUnchanged(t *testing.T) {
	err := runCLI(t, "classify", "-a", "-t", t.TempDir(), "foo")
	require.ErrorIs(t, err, tree.ErrNotPortsTree)

	root := selectorTree(t, "devel/ivy")
	err = runCLI(t, "classify", "-a", "-t", root, "foo")
...
func TestClassifySelectorErrors(t *testing.T) {
	...
	err := runCLI(t, "classify", "-t", root, "maintainer:nobody")
	...
	err = runCLI(t, "classify", "-t", root, "maintainer:me")
	require.Error(t, err)
	...
	err = runCLI(t, "classify", "-t", root, "category:nosuch")
...
func TestOutdatedResolvesTheMaintainerForms(t *testing.T) {
	...
	err := runCLI(t, "outdated", "-t", root, "maintainer:nobody")
	require.ErrorIs(t, err, sweep.ErrNoMaintainer)

	err = runCLI(t, "outdated", "-t", root, "maintainer:me")
	require.Error(t, err)
	assert.NotContains(t, err.Error(), "is wired",
...
func TestSelectorReadsCategorySlashPortAsThePort(t *testing.T) {
	...
	svc := &Services{TreeRoot: root, Tools: testFinder(), Out: &out, Err: &errb, Now: time.Now}
	ctx := context.Background()
	// The composition root opens what a road DECLARES, so a test driving
	// the resolver directly declares the same thing the report verbs do.
	require.NoError(t, svc.Acquire(ctx, app.Needs{Tree: true}))
	t.Cleanup(svc.Close)

	// The report road: classify and outdated resolve through this.
	targets, err := resolveTargets(ctx, svc, false, []string{"sysutils/jq"})
	...
	res, err := resolveSelector(ctx, svc, "sysutils/jq")
	require.NoError(t, err)
	...
	// not have moved.
	targets, err = resolveTargets(ctx, svc, false, []string{"sysutils"})
	require.NoError(t, err)
