...
func NewRoot(config app.Config) (*cobra.Command, error) {
	...
	root := &cobra.Command{
		...
		Long:          "Dockhand prepares, verifies, and publishes MacPorts changes.\nPrepare version and revision bumps, verify committed ports, resume jobs, and run driver cycles. Automatic selection supports GitHub sources with stable numeric versions and a tags livecheck. Publication is under construction.",
		Args:          cobra.NoArgs,
		SilenceErrors: true,
		SilenceUsage:  true,
