	...
	for _, spec := range []struct {
		...
		command := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				if build.dependents && (skipVerify || options.Diff) {
					return fmt.Errorf("--dependents requires a build; it does not go with --to branch, --unverified, or --diff")
				}
				...
				if publishing && !options.Diff {
					...
					return fmt.Errorf("--remote, --upstream, --base, and --refresh-body apply to the PR, so they need --to pr, or GitHub verification")
				}
				if !options.Diff {
