...
func TestBodyUsesSelectedEvidenceAndGeneratedIdentity(t *testing.T) {
	...
	body := publicationBody(content, change, source, attempt, nil)
	...
	require.NotContains(t, publicationBody(content, change, source, attempt, nil), " -s ")
	attempt.Evidence.Steps[2].Command = append([]string{"/opt/local/bin/port", "-s"}, attempt.Evidence.Steps[2].Command[1:]...)
	require.Contains(t, publicationBody(content, change, source, attempt, nil), "port -s -N")
	for _, generated := range []record.ObjectID{"", record.ObjectID(strings.Repeat("b", 40))} {
		change.GeneratedCommit = generated
		require.Contains(t, publicationBody(content, change, source, attempt, nil), "[ ] Squashed")
	}
}
...
func TestBodyDoesNotInventTestsOrEnvironmentForOlderEvidence(t *testing.T) {
	...
	for _, reason := range []string{"", "Skipped by request", "Port declares no test phase"} {
		...
		body := publicationBody(record.PublicationContent{Title: "fixture: update"}, record.Change{}, record.Source{}, attempt, nil)
	...
	for _, guest := range []*record.GuestEnvironment{nil, {}, {NoActivePorts: true}, {NoForeignPackageManagers: true}} {
		...
		require.NotContains(t, publicationBody(record.PublicationContent{}, record.Change{}, record.Source{}, attempt, nil), "(pristine)")
	}
}
...
func TestBodyReportsWorkflowSuccessWithoutInventingPortPhases(t *testing.T) {
	...
	body := publicationBody(record.PublicationContent{Title: "fixture: update"}, record.Change{}, record.Source{}, record.Attempt{ID: "attempt", Evidence: evidence}, nil)
...
func TestBodyDisclosesAnUnverifiedPublication(t *testing.T) {
	...
	change := record.Change{GeneratedCommit: source.Commit}
	content := record.PublicationContent{Title: "fixture: update to 2", Body: "Useful explanation"}
	body := publicationBody(content, change, source, record.Attempt{}, nil)
...
func TestBodyReportsAnAdvisoryTestFailure(t *testing.T) {
	...
	body := publicationBody(record.PublicationContent{Title: "fixture: update to 2"}, record.Change{}, source, attempt, nil)
...
func TestTestedOnTablesTheEnvironmentAndBulletsTheProvider(t *testing.T) {
	...
	body := publicationBody(record.PublicationContent{Title: "jc: update to 1.26.0"}, record.Change{}, record.Source{}, attempt, nil)
	...
	require.NotContains(t, publicationBody(record.PublicationContent{}, record.Change{}, record.Source{}, attempt, nil), "| :--- | :--- |")
}
...
// A changed PortGroup beside the port gets its own section ahead of the
// verdict, naming what loads it and that only the port was built.
func TestBodyNamesSharedFilesTheVerificationDidNotCover(t *testing.T) {
	t.Parallel()
	content := record.PublicationContent{Title: "Okapi: update to 1.48.0", Body: "Explain"}
	change := record.Change{InitiatingTarget: "Okapi", Targets: []record.Target{{Name: "Okapi", Portfile: "aqua/Okapi/Portfile"}}}
	body := publicationBody(content, change, record.Source{}, record.Attempt{}, []record.SharedFile{{Path: "_resources/port1.0/group/java-1.0.tcl", Loaders: []string{"aqua/Okapi/Portfile", "java/jarviz/Portfile"}}})
	require.Contains(t, body, "###### Shared files\n\n- changes PortGroup java-1.0, loaded by 2 ports; only Okapi was built\n")
	require.Less(t, strings.Index(body, "###### Shared files"), strings.Index(body, "###### Tested on"))
	require.NotContains(t, publicationBody(content, change, record.Source{}, record.Attempt{}, nil), "Shared files")
}
