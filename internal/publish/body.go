...
// unrunCause says why a publication carries no run at all, and the four
// arms are four different facts.
//
// A change bound for the branch was minted with --no-verify, so nobody
// ever asked for a verdict. A change with attempts, none of them over
...
// shipped body printed for every one of these.
//
...
func unrunCause(f Facts) string {
	switch {
	case f.Change.Destination == record.ToBranch:
