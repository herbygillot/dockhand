...
func TestTheProofIsTakenAgainstWhatMovedAndNotAgainstEverything(t *testing.T) {
	...
	abi := Delta(measured(before, after))
	require.Equal(t, Changed, abi.Verdict)
...
func TestBrokeNamesWhatADependentWouldHaveRecorded(t *testing.T) {
	...
	assert.Empty(t, Result{}.Broke(), "a measurement with no break has nothing to prove a link against")
}
