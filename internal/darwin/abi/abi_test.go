...
func measured(before, after *artifact.Manifest) Input {
	return Input{
		Port: "libwidget", Portdir: "devel/libwidget", Described: true, FromSource: true,
		Baseline: before, Source: Archive, Candidate: after,
	}
}
...
func TestARenamedInstallNameIsABreakAndTheCriterionSaysBetweenWhat(t *testing.T) {
	a := Delta(measured(widgetBefore(), widgetAfter()))

	require.Equal(t, Changed, a.Verdict)
	...
	assert.Equal(t, "/opt/local/lib/libwidget.2.dylib", a.Changes[0].From)
	assert.Equal(t, "/opt/local/lib/libwidget.3.dylib", a.Changes[0].To)
	...
	// The headline travels with the result, because the proposal that
	// reads it takes nothing else that could name one.
	assert.Equal(t, "libwidget", a.Port)
	assert.Equal(t, "devel/libwidget", a.Portdir)
}
...
func TestAFatFileWhoseSlicesDisagreeIsNotMeasured(t *testing.T) {
	...
	a := Delta(measured(before, widgetAfter()))
	...
	assert.Contains(t, a.Why, "not measured", "what could not be looked at is in the sentence")
}

func TestThreeFilesAreOneLogicalLibrary(t *testing.T) {
	t.Run("an unchanged install is unchanged", func(t *testing.T) {
		a := Delta(measured(brotli(), brotli()))

		assert.Equal(t, Unchanged, a.Verdict)
		assert.False(t, a.Broken())
		assert.Empty(t, a.Changes)
		assert.Contains(t, a.Why, "no install name, compatibility version or library moved")
	})
	...
	t.Run("files that disagree about one name are not compared", func(t *testing.T) {
		...
		a := Delta(measured(odd, brotli()))
		require.Len(t, a.Unmeasured(), 1)
		assert.Equal(t, "/opt/local/lib/libbrotlicommon.1.dylib", a.Unmeasured()[0].Subject)
		assert.Contains(t, a.Why, "two compatibility versions, 1.0.0 and 2.0.0")
		assert.Equal(t, Unchanged, a.Verdict, "nothing was measured about it, so nothing was disproved")
	})
...
func TestALibraryThatGainsAMajorIsNotABreak(t *testing.T) {
	...
	t.Run("both sides carry both", func(t *testing.T) {
		a := Delta(measured(both, both))
		assert.Equal(t, Unchanged, a.Verdict)
		assert.Empty(t, a.Changes)
	})
	t.Run("a major is added", func(t *testing.T) {
		a := Delta(measured(one, both))
		assert.Equal(t, Unchanged, a.Verdict, "no dependent can have recorded a name that did not exist")
		assert.Empty(t, a.Changes)
	})
	t.Run("a major is dropped", func(t *testing.T) {
		a := Delta(measured(both, one))
...
func TestCompatibilityVersionNarrowsAndWidens(t *testing.T) {
	...
	t.Run("backwards is a break", func(t *testing.T) {
		a := Delta(measured(at("30.0.0"), at("29.0.0")))
		require.Len(t, a.Changes, 1)
		...
		assert.Equal(t, Changed, a.Verdict)
	...
	t.Run("forwards is not", func(t *testing.T) {
		...
		a := Delta(measured(at("4.0.0"), at("30.0.0")))
		require.Len(t, a.Changes, 1)
...
func TestAnRpathInstallNameIsNotComparable(t *testing.T) {
	...
	a := Delta(measured(at("a9b31f558d66d404"), at("0c3e1f2b7d5a4e91")))
	...
	assert.Equal(t, Unavailable, a.Verdict, "nothing was measured, which is not the same as nothing having moved")
	assert.False(t, a.Broken())
	...
	assert.Contains(t, a.Why, "could be compared")
	assert.Contains(t, a.Why, "resolved by the dependent", "and it names what it could not read")
}
...
func TestABIUnavailableTellsTheThreeAbsencesApart(t *testing.T) {
	...
	t.Run("no manifester", func(t *testing.T) {
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget"})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.False(t, a.Broken())
		assert.Contains(t, a.Why, "cannot describe an installation")
	})
	t.Run("nothing was installed", func(t *testing.T) {
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget", Described: true,
			Baseline: widgetBefore(), Source: Archive})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Contains(t, a.Why, "no installation to measure")
	})
	t.Run("no baseline, in the environment's own words, and no remedy that does not exist", func(t *testing.T) {
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget", Described: true,
			Candidate: widgetAfter(),
			Source:    NoBaseline,
			Reason:    "Error: Failed to unarchive libwidget: Archive for libwidget 2.4.1_0 not found, required when binary-only is set!",
		})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Contains(t, a.Why, "no baseline for libwidget")
		assert.Contains(t, a.Why, "required when binary-only is set", "MacPorts' own refusal, verbatim")
		assert.Contains(t, a.Why, "nothing banks one yet",
		...
		assert.NotContains(t, a.Why, "banks one`")
		assert.NotContains(t, a.Why, "dockhand verify")
	})
	t.Run("a refusal still names the port the environment reported", func(t *testing.T) {
		...
		a := Delta(Input{Described: true, Portdir: "devel/libwidget",
			Candidate: widgetAfter(), Source: NoBaseline})
		assert.Equal(t, "libwidget", a.Port)
		assert.Contains(t, a.Why, "no baseline for libwidget")
	})
	t.Run("a banked source with no banked value is its own refusal", func(t *testing.T) {
		...
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget", Described: true,
			Candidate: widgetAfter(), Source: Banked})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Contains(t, a.Why, "recorded as banked and no banked manifest was supplied")
		assert.NotContains(t, a.Why, "no baseline for libwidget",
	...
	t.Run("an empty baseline is never read as every library removed", func(t *testing.T) {
		...
		a := Delta(Input{Port: "libwidget", Described: true,
			Baseline: &artifact.Manifest{Port: "libwidget"}, Source: NoBaseline,
			Candidate: widgetAfter(),
		})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Empty(t, a.Changes)
...
func TestWhatTheMeasurementCannotSeeTravelsWithIt(t *testing.T) {
	...
	assert.Contains(t, Limits, "symbols are removed")
	assert.Contains(t, Limits, "invisible to otool")

	assert.Equal(t, Limits, Delta(measured(widgetBefore(), widgetAfter())).Limits())
	assert.Equal(t, Limits, Delta(measured(brotli(), brotli())).Limits(),
...
func TestOneSideWithNoLibraryIsRefusedRatherThanConcluded(t *testing.T) {
	...
	t.Run("an empty before compares as nothing moved", func(t *testing.T) {
		...
		// happened.
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget", Described: true,
			Baseline: empty, Source: Archive, Candidate: widgetAfter()})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Contains(t, a.Why, "describes 2 files and no library")
		assert.NotContains(t, a.Why, "no install name, compatibility version or library moved")
	})

	t.Run("an empty after compares as every library removed", func(t *testing.T) {
		a := Delta(Input{Port: "libwidget", Portdir: "devel/libwidget", Described: true,
			Baseline: widgetBefore(), Source: Archive, Candidate: empty})
		assert.Equal(t, Unavailable, a.Verdict)
		assert.Contains(t, a.Why, "would read as removed")
	...
	t.Run("two installations with no library at all still compare", func(t *testing.T) {
		...
		a := Delta(Input{Port: "widget-doc", Described: true,
			Baseline: empty, Source: Archive, Candidate: empty})
		assert.Equal(t, Unchanged, a.Verdict)
	})
}
...
func TestBothHalvesOfTheCriterionNameTheirSource(t *testing.T) {
	in := measured(widgetBefore(), widgetAfter())
	in.FromSource = true
	assert.Contains(t, Delta(in).Why, "(binary archive) and @3.0.0_0+universal (built from source)")

	in.FromSource = false
	assert.Contains(t, Delta(in).Why, "(binary archive) and @3.0.0_0+universal (source not recorded)",
		"an assumed provenance and a stated absence are different things to a reviewer, and only one can be noticed")
}

// The baseline provenance is one vocabulary with two spellings, and
// this is the only place they are allowed to meet.
//
// The provider says where a baseline came from and the record stores
// the word; this package reasons about it and, under the ruled import
// direction, may not import the provider to do so — its one edge goes
// to artifact. So the three constants are declared here as well, and
// this test is what keeps a second declaration from becoming a second
// answer: a provider that renamed its own word would fail here rather
// than silently produce criteria that say "an unnamed source" for every
// measurement dockhand takes.
//
// The import is the test's alone. The package under test cannot make
// this call, which is exactly the property being protected.
func TestTheBaselineSourcesAreTheProvidersOwnWords(t *testing.T) {
	assert.Equal(t, Archive, Source(verify.BaselineArchive))
	assert.Equal(t, Banked, Source(verify.BaselineBanked))
	assert.Equal(t, NoBaseline, Source(verify.BaselineNone))
}
