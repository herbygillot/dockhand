...
const checkpointColumns = "number, branch_id, before_head, after_head, at, restored_at"

func scanCheckpoint(row interface{ Scan(...any) error }) (model.Checkpoint, error) {
	var c model.Checkpoint
	var at int64
	var restored sql.NullInt64
	if err := row.Scan(&c.Number, &c.Branch, &c.Before, &c.After, &at, &restored); err != nil {
...
func (t *tx) AddCheckpoint(c model.Checkpoint) error {
	...
	_, err = t.exec("INSERT INTO checkpoints(repository_id, "+checkpointColumns+") VALUES(?,?,?,?,?,?,?)",
		t.repo, c.Number, c.Branch, c.Before, c.After, millis(c.At), nullableMillis(c.RestoredAt))
	return err
}
