...
// PatchWontRelocate WAS HERE, and it is deleted rather than kept for a
// caller that no longer exists. A patch that does not carry over to the
// new source ended the whole bump; ruled 8 September 2026, it is a
// Proposed finding on a minted branch instead — bump.FindingPatchUnrelocated
// — because the person who meets it can do the one thing dockhand may
// not, which is judge what the patch was for.
//
// The zero-fuzz rule this type argued for is unchanged and now lives
// where the move is made: dockhand relocates a hunk only where its
// before-block occurs exactly once, verbatim.
