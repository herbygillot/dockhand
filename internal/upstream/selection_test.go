...
func TestRequestedVersionUsesConfirmedTagConvention(t *testing.T) {
	for _, pattern := range []upstream.TagPattern{{Prefix: "v"}, {Prefix: "jq-"}, {Prefix: "release/", Suffix: "-stable"}, {}} {
		...
		observed := []upstream.Release{{Tag: tag, URL: "https://example.invalid/release"}}
		for _, requested := range []string{"1.8.1", tag} {
			...
			require.Equal(t, "1.8.1", selected.Release.Version)
			require.Equal(t, tag, selected.Release.Tag)
...
func TestReleaseSelectionDoesNotGuessPastAmbiguousOrMissingEvidence(t *testing.T) {
	...
	_, err := upstream.MatchRelease("2", pattern, []upstream.Release{{Version: "2", Tag: "2"}, {Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrReleaseAmbiguous)
	require.ErrorContains(t, err, "specify the exact tag")
	_, err = upstream.MatchRelease("2", pattern, []upstream.Release{{Tag: "2"}, {Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrReleaseAmbiguous)
	_, err = upstream.MatchRelease("v2", pattern, []upstream.Release{{Version: "3", Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrTagPattern)

	_, err = upstream.MatchRelease("v2", pattern, []upstream.Release{{Tag: "vv2"}})
	require.ErrorIs(t, err, upstream.ErrReleaseMissing, "explicit prefix must not be doubled")
	_, err = upstream.MatchRelease("3", pattern, []upstream.Release{{Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrReleaseMissing)
	_, err = upstream.MatchRelease("v2", nil, []upstream.Release{{Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrTagPattern)
	selected, err := upstream.MatchRelease("2-rc1", pattern, []upstream.Release{{Tag: "v2-rc1", Prerelease: true}})
	require.NoError(t, err, "an explicit prerelease is not automatic latest selection")
	require.True(t, selected.Release.Prerelease)
	selected, err = upstream.MatchRelease("2", nil, []upstream.Release{{Version: "2", URL: "https://example.invalid/2.tar.gz"}})
	require.NoError(t, err)
	require.Empty(t, selected.Release.Tag, "archive releases need not invent tags")
	_, err = upstream.MatchRelease("2", pattern, []upstream.Release{{Version: "3", Tag: "v2"}})
	require.ErrorIs(t, err, upstream.ErrTagPattern)
