...
// ResourcesDir is the tree's own resource directory, spelled once in
// macports. A staged tree without it is not a ports tree:
// portarchivefetch resolves archive_sites.tcl under the port's own tree
// with the fallback disabled, so a port served without it can reach no
// archive at all.
const ResourcesDir = macports.ResourcesDir
