...
func TestPublicationUpdatesCorrectedCommitAndPreservesHumanPRBody(t *testing.T) {
	...
	require.True(t, strings.HasPrefix(request.Spec.Publication.Desired.Body, "Human review context to preserve\n\n###### Tested on"), "the person's words stay first and gain the section: %q", request.Spec.Publication.Desired.Body)
	...
	require.True(t, strings.HasPrefix(status.PullRequests[0].Body, "Human review context to preserve\n\n###### Tested on"))
