...
The CLI uses Cobra v1.10.2, matching Dockhand v1. Run `dockhand help <command>` for command-specific help; `usage` is an alias for `help`. Cobra supplies shell completion generation through `dockhand completion`. Use `dockhand status --json` for the typed ledger projection. Action commands remain unfinished; the workflow submission API accepts queued work but does not advance it.
