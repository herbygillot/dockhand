> [!IMPORTANT]
> **`main` is being rebuilt as dockhand v3** ([design](docs/design-v3.md), [roadmap](docs/roadmap.md)). Until v3's commands land, the working tool is **v2**, at the tag `v2-final`, and this page describes it. Build it beside this checkout with `git worktree add ../dockhand-v2 v2-final && make -C ../dockhand-v2 build BINARY="$HOME/.local/bin/dockhand-v2"`. v3 uses a new database, `~/.dockhand/dockhand.db`, and never reads v2's `state.db`.
